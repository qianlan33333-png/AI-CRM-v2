...
type AttachmentService struct {
	...
	automation automationport.AttachmentReferenceReader
	contact    contactport.AttachmentReferenceReader
	radar      radarport.AttachmentReferenceReader
	events     eventport.Appender
	now        func() time.Time
}

func NewAttachmentService(uow platformport.UnitOfWork, store AttachmentStore, events eventport.Appender) *AttachmentService {
	return &AttachmentService{uow: uow, store: store, events: events, now: time.Now}
}

func NewAttachmentServiceWithReferences(uow platformport.UnitOfWork, store AttachmentStore, automation automationport.AttachmentReferenceReader, contact contactport.AttachmentReferenceReader, radar radarport.AttachmentReferenceReader, events eventport.Appender) *AttachmentService {
...
func (service *AttachmentService) appendAttachmentEvent(ctx context.Context, action string, attachment mediaport.Attachment, actor int64, key string, now time.Time) error {
	...
	_, err = service.events.Append(ctx, eventport.Event{Type: eventType, Payload: payload, OccurredAt: now, IdempotencyKey: "media.attachment_" + action + ":" + hex.EncodeToString(digest[:])})
	return err
}
...
func (service *AttachmentService) appendAttachmentDeleteEvent(ctx context.Context, actor int64, key string, now time.Time, attachmentID int64) error {
	...
	_, err = service.events.Append(ctx, eventport.Event{Type: "media.attachment_deleted", Payload: payload, OccurredAt: now, IdempotencyKey: "media.attachment_deleted:" + hex.EncodeToString(digest[:])})
	return err
}
