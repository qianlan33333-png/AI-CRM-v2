...
// remains unchanged.
func (s *Service) UpdateImageMetadata(ctx context.Context, command ImageMetadataUpdateCommand) (ImageMetadata, error) {
	...
	err = s.uow.Within(ctx, func(txCtx context.Context) error {
		...
		if _, err := s.events.Append(txCtx, eventport.Event{
