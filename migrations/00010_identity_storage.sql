...
CREATE TABLE identities (
  ...
  CONSTRAINT identities_kind CHECK (kind IN ('wecom_external_userid', 'unionid', 'mp_openid', 'oa_openid', 'alipay_user_id', 'phone', 'ext')),
  CONSTRAINT identities_scope_not_blank CHECK (btrim(scope) <> '' AND char_length(scope) <= 256),
  CONSTRAINT identities_normalized_not_blank CHECK (btrim(normalized_value) <> '' AND char_length(normalized_value) <= 1024),
  ...
  CONSTRAINT identities_bound_consistency CHECK ((customer_id IS NULL AND bound_at IS NULL) OR (customer_id IS NOT NULL AND bound_at IS NOT NULL)),
...
CREATE TABLE pending_events (
  ...
  CONSTRAINT pending_events_identity_ids CHECK (cardinality(identity_ids) > 0 AND array_position(identity_ids, NULL) IS NULL),
  ...
  CONSTRAINT pending_events_resolution CHECK ((state = 'pending' AND resolved_at IS NULL) OR (state <> 'pending' AND resolved_at IS NOT NULL)),
...
CREATE TABLE identity_operation_receipts (
  ...
  CONSTRAINT identity_receipts_operation CHECK (operation IN ('bind', 'ingest', 'merge_review_approve', 'merge_review_reject')),
  CONSTRAINT identity_receipts_scope CHECK (btrim(idempotency_scope) <> '' AND char_length(idempotency_scope) <= 256),
  ...
  CONSTRAINT identity_receipts_result_status CHECK (result_status IS NULL OR result_status IN ('bound', 'already_bound', 'merged', 'manual_review', 'rejected', 'attributed', 'pending', 'conflict', 'approved')),
  CONSTRAINT identity_receipts_completed_result CHECK (
    (state = 'in_progress' AND result_status IS NULL AND result_customer_id IS NULL AND result_merge_audit_id IS NULL AND result_pending_event_id IS NULL AND result_event_id IS NULL AND result_policy_version IS NULL AND completed_at IS NULL)
    OR
    (state = 'completed' AND result_status IS NOT NULL AND completed_at IS NOT NULL AND (result_customer_id IS NULL OR result_customer_id > 0) AND (result_merge_audit_id IS NULL OR result_merge_audit_id > 0) AND (result_pending_event_id IS NULL OR result_pending_event_id > 0) AND (result_event_id IS NULL OR result_event_id > 0) AND (result_policy_version IS NULL OR (btrim(result_policy_version) <> '' AND char_length(result_policy_version) <= 200)) AND ((operation = 'bind' AND result_status IN ('bound', 'already_bound', 'merged', 'manual_review', 'rejected')) OR (operation = 'ingest' AND result_status IN ('attributed', 'pending', 'conflict')) OR (operation = 'merge_review_approve' AND result_status = 'approved') OR (operation = 'merge_review_reject' AND result_status = 'rejected')))
  ),
...
BEGIN
  IF TG_OP = 'DELETE' OR OLD.state = 'completed' THEN
    RAISE EXCEPTION 'identity operation receipts are immutable after completion' USING ERRCODE = '55000';
  END IF;
  IF NEW.state <> 'completed' OR NEW.operation IS DISTINCT FROM OLD.operation OR NEW.idempotency_scope IS DISTINCT FROM OLD.idempotency_scope OR NEW.key_digest IS DISTINCT FROM OLD.key_digest OR NEW.command_schema_version IS DISTINCT FROM OLD.command_schema_version OR NEW.payload_hmac IS DISTINCT FROM OLD.payload_hmac OR NEW.payload_hmac_key_version IS DISTINCT FROM OLD.payload_hmac_key_version OR NEW.result_schema_version IS DISTINCT FROM OLD.result_schema_version OR NEW.created_at IS DISTINCT FROM OLD.created_at THEN
    RAISE EXCEPTION 'invalid identity operation receipt transition' USING ERRCODE = '55000';
  END IF;
  RETURN NEW;
END;
$$;
...
AS $$
BEGIN
  RAISE EXCEPTION 'customer_merges is append-only' USING ERRCODE = '55000';
END;
$$;
-- +goose StatementEnd
