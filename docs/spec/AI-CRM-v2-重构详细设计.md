...
3. **侧边栏/菜单/权限缓存**：P2 尚无服务端菜单/未读聚合 API，先对已冻结的
   `GET /api/v1/auth/session` 结果在浏览器进程内缓存 60s，只缓存经闭集校验的
   principal/role 与由冻结 capability 推导的可见菜单；不缓存 bearer、不写
   localStorage/sessionStorage，logout 主动失效，未知角色或异常响应一律不缓存并
   fail-closed。后续一旦引入服务端菜单/未读聚合 API，再在 Go API 进程增加 LRU
   （TTL 60s，权限/菜单变更主动失效）；不得为提前满足缓存形态而伪造聚合端点。
   侧边栏慢的典型病因就是每次渲染重复权限判定和聚合查询，两级策略都必须保证
   缓存只减少重复计算，不能成为授权边界或扩大服务端 RBAC。
