...
type projectionStub struct {
	settings []configstore.ProjectionSetting
	audits   []configstore.ProjectionAudit
	err      error
}

func (stub *projectionStub) ListAppSettings(context.Context) ([]configstore.ProjectionSetting, error) {
	return stub.settings, stub.err
}
func (stub *projectionStub) ListAppSettingsAudit(context.Context) ([]configstore.ProjectionAudit, error) {
...
func TestSettingsProjectionFreezesTwelveKeyDTOAndSecretBoundary(t *testing.T) {
	...
	repo := &projectionStub{
		settings: []configstore.ProjectionSetting{{Key: configport.WeComCorpID, Value: []byte(`"corp"`), UpdatedAt: updated, LastActionType: "update", LastModifiedBy: "admin:7", LastModifiedAt: &updated}},
		audits: []configstore.ProjectionAudit{
