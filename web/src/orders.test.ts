...
function refund(extra: Record<string, unknown> = {}) {
  return {
    ...
    external_effect_state: "pending_external_gate",
...
describe("local order overview read boundary", () => {
  it("uses only the canonical same-origin order list request", async () => {
    ...
    await expect(loadOrders(client)).resolves.toMatchObject({
      status: "loaded",
    });
  ...
  it("switches server-side local provider workspaces without using provider endpoints", async () => {
    const client = transport(
      200,
      page([item({ provider: "alipay", provider_label: "支付宝" })]),
    );
    await expect(loadOrders(client, 0, "alipay")).resolves.toMatchObject({
      status: "loaded",
      page: { providerScope: "alipay" },
    });
    expect(client.list).toHaveBeenCalledWith(
    ...
    await expect(
      loadOrders(transport(200, page()), 0, "alipay"),
    ).resolves.toEqual({ status: "invalid" });
  });

  it("creates and rereads one closed local CSV receipt while discarding sensitive content", async () => {
    ...
    await expect(
      createLocalOrderExport(
        ...
        "orders",
        "c".repeat(43),
        "order-export:00000000-0000-4000-8000-000000000000",
      ),
    ).resolves.toEqual({
      status: "completed",
      value: {
    ...
    expect(getExport).toHaveBeenCalledWith("exp_0000000000000001", {
      credentials: "same-origin",
    });
  });

  it("fails closed and does not retry when a local export receipt or reread drifts", async () => {
    for (const [created, reread] of [
      [exportReceipt({ extra: true }), exportReceipt()],
      [exportReceipt(), exportReceipt({ content_text: "different" })],
      [
        exportReceipt({ download_url: "https://outside.example/file.csv" }),
        exportReceipt(),
      ],
      [exportReceipt({ created_at: "2026-02-30T00:00:00Z" }), exportReceipt()],
    ] as const) {
      const createExport = vi.fn(async () => ({ status: 200, data: created }));
      const getExport = vi.fn(async () => ({ status: 200, data: reread }));
      const result = await createLocalOrderExport(
        {
          ...transport(200, page()),
          createExport,
          getExport,
      ...
      expect(getExport.mock.calls.length).toBeLessThanOrEqual(1);
    }
    const unavailable = vi.fn(async () => ({ status: 503, data: {} }));
    await expect(
      createLocalOrderExport(
        {
          ...transport(200, page()),
          createExport: unavailable,
          getExport: vi.fn(),
        } as OrdersTransport,
        "orders",
        "c".repeat(43),
  ...
  it("treats every readback failure after a successful POST as outcome unknown", async () => {
    for (const status of [401, 403, 404, 503]) {
      const createExport = vi.fn(async () => ({ status: 200, data: exportReceipt() }));
      const getExport = vi.fn(async () => ({ status, data: {} }));
      const result = await createLocalOrderExport(
        { ...transport(200, page()), createExport, getExport } as OrdersTransport,
        "orders", "c".repeat(43), "order-export:00000000-0000-4000-8000-000000000000",
      );
      expect(result).toEqual(status === 401 ? { status: "unknown", unauthenticated: true } : { status: "unknown" });
      expect(createExport).toHaveBeenCalledOnce();
  ...
  it("fails closed for malformed pages and unsafe or extra persisted facts", async () => {
    ...
    for (const data of [
      page([item({ unexpected: true })]),
      page([item({ amount_yuan: "19.9" })]),
      page([item({ created_at: "2026-02-31T00:00:00Z" })]),
      page([item({ provider: "stripe" })]),
      page([item({ detail_url: "//outside.example/orders/1" })]),
      page([item({ userid: "a", unionid: "b" })]),
      page(duplicate, { total: 2 }),
      page([item()], { total: 2, has_more: false }),
      page([item()], { limit: 20 }),
      page([], { total: 1, has_more: true }),
      { ...page(), extra: true },
    ]) {
      await expect(loadOrders(transport(200, data))).resolves.toEqual({
        status: "invalid",
      });
    }
  });
  ...
  it("maps read failures without retry and preserves only safe offset navigation", async () => {
    await expect(loadOrders(transport(401, {}))).resolves.toEqual({
      status: "unauthenticated",
    });
    await expect(loadOrders(transport(403, {}))).resolves.toEqual({
      status: "forbidden",
    });
    await expect(loadOrders(transport(503, {}))).resolves.toEqual({
      status: "unavailable",
    });
    await expect(loadOrders(transport(200, page()), 1)).resolves.toEqual({
      status: "invalid",
    });
    expect(
      previousOrderOffset({
  ...
  it("uses one canonical same-origin detail GET and retains only mirrored local facts", async () => {
    const client = transport(
      200,
      page([item({ external_userid: "wmid-1" })]),
      200,
      detail({ external_userid: "wmid-1" }),
    );
    const loaded = await loadOrders(client);
    if (loaded.status !== "loaded") throw new Error("expected local item");
    await expect(
      loadOrderDetail(client, loaded.page.items[0]),
    ).resolves.toEqual({
      status: "loaded",
      detail: {
        id: 17,
  ...
  it("fails closed for direct-detail envelopes, malformed fields, and safe-record drift", async () => {
    ...
    for (const data of [
      ...
      detail({ id: 0 }),
      detail({ refundable_amount_total: 1991 }),
      detail({ refundable_amount_total: -1 }),
      detail({ order_no: "M-2" }),
      detail({ provider: "alipay" }),
      detail({ status: "created" }),
      detail({ external_userid: "wmid-1", unionid: "u-1" }),
    ]) {
      await expect(
        loadOrderDetail(
          transport(200, page(), 200, data),
          loaded.page.items[0],
        ),
      ).resolves.toEqual({ status: "invalid" });
    }
  ...
  it("drops opaque identity fields from the validated list and detail projection", async () => {
    const listed = await loadOrders(
      transport(200, page([item({ external_userid: "wmid-1" })])),
    );
    if (listed.status !== "loaded") throw new Error("expected local item");
    await expect(
      loadOrderDetail(
        transport(200, page(), 200, detail({ external_userid: "wmid-2" })),
        listed.page.items[0],
      ),
  ...
  it("keeps accepted local detail-url text inert without inventing stricter URL semantics", async () => {
    for (const detailUrl of [
      ...
      const loaded = await loadOrders(
        transport(200, page([item({ detail_url: detailUrl })])),
      );
      if (loaded.status !== "loaded") throw new Error("expected local item");
      await expect(
        loadOrderDetail(
          transport(200, page(), 200, detail({ detail_url: detailUrl })),
          loaded.page.items[0],
        ),
      ).resolves.toMatchObject({ status: "loaded" });
    }
        ...
        items: [
          {
            ...
            currency: "CNY",
            status: "pending_external_gate",
            externalEffectState: "pending_external_gate",
            createdAt: "2026-08-19T00:00:00Z",
          },
        ],
        total: 1,
  ...
  it("reads the existing one-item snapshot and projects only safe product fields", async () => {
    const client = transport(
      ...
      refundPage(),
      200,
      { items: [item({ external_userid: "wmid-hidden" })] },
    );
    ...
    expect(client.items).toHaveBeenCalledWith(
      "M-1",
      { provider: "wechat" },
      { credentials: "same-origin" },
    );
  ...
  it("fails closed for item-envelope drift and filters only existing safe page facts", async () => {
    ...
    await expect(
      loadOrderItems(
        transport(200, page(), 200, detail(), 200, refundPage(), 200, {
          items: [item()],
          extra: true,
        }),
        listed.page.items[0],
    ...
    expect(
      filterSafeOrders(listed.page.items, {
        keyword: "SKU",
        provider: "wechat",
        status: "paid",
      }),
    ...
    const local = await loadLocalRefunds(
      transport(200, page(), 200, detail(), 200, refundPage()),
    );
    if (local.status !== "loaded") throw new Error("expected refunds");
    expect(
      filterSafeRefunds(local.page.items, {
        keyword: "M-1",
        provider: "wechat",
        status: "pending_external_gate",
      }),
  ...
  it("fails closed for all local refund DTO, page, and external-effect contract drift", async () => {
    const invalid = [
      refund({ unexpected: true }),
      refund({ id: 0 }),
      refund({ order_id: 0 }),
      refund({ provider: "stripe" }),
      refund({ order_no: "" }),
      refund({ transaction_id: "" }),
      refund({ refund_id: "legacy-1" }),
      refund({ out_refund_no: "legacy-1" }),
      refund({ refund_amount_total: 0 }),
      refund({ currency: "USD" }),
      refund({ reason: "" }),
      refund({ status: "succeeded" }),
      refund({ external_effect_state: "succeeded" }),
      refund({ auto_retry_allowed: true }),
    ...
    for (const data of [
      ...
      refundPage([refund()], { total: 2, has_more: false }),
      refundPage([refund()], { limit: 20 }),
      { ...refundPage(), unexpected: true },
    ]) {
      await expect(
        loadLocalRefunds(transport(200, page(), 200, detail(), 200, data)),
      ).resolves.toEqual({ status: "invalid" });
    }
  });
  ...
  it("maps local refund-history failures without retry and keeps its offset boundary", async () => {
    ...
    expect(
      nextLocalRefundOffset({ ...localPage, offset: 0, hasMore: true }),
    ).toBe(50);
  });

  it("reads only a closed local external-effect projection and discards receipts", async () => {
    const externalEffects = vi.fn(async () => ({
      status: 200,
      data: {
        items: [
          {
            ...
            order_id: 17,
            provider: "wechat",
            effect_kind: "external_push",
            state: "outcome_unknown",
            auto_retry_allowed: false,
            provider_receipt: "hidden",
    ...
    const client: OrdersTransport = {
      ...transport(200, page(), 200, detail()),
      externalEffects,
    };
    const listed = await loadOrders(client);
    if (listed.status !== "loaded") throw new Error("expected order");
    const loaded = await loadOrderDetail(client, listed.page.items[0]);
    if (loaded.status !== "loaded") throw new Error("expected detail");
    await expect(
      loadLocalExternalEffects(client, loaded.detail),
    ).resolves.toEqual({
      status: "loaded",
      page: {
        items: [
    ...
    expect(externalEffects).toHaveBeenCalledWith("M-1", {
      credentials: "same-origin",
    });
  });
});
