...
[[ "$latest" = '143' ]] || fail "final schema must be 143, found ${latest:-none}"

grep -Fq '"from": 132' "$manifest" || fail 'manifest must start from schema 132'
grep -Fq '"to": 143' "$manifest" || fail 'manifest must end at schema 143'
...
if [[ "$mode" = '--render' ]]; then
  ...
  printf 'final-v1-domain-migration-plan: PASS (schema=132->143 domains=%s)\n' "$domain_count"
fi
