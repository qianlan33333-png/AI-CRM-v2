...
const tagWriteOptions = (metadata: { idempotency_key: string }) => apiRequestOptions({ headers: { 'Idempotency-Key': metadata.idempotency_key } });
export async function saveTagGroupDto(input: { id?: number; name: string; firstTag?: string }): Promise<TagGroup> { const metadata = writeMeta(); const opt = tagWriteOptions(metadata); if (input.id == null) { if (!input.firstTag) throw new Error('新建标签组必须提供首个标签'); const result = obj(await call(createLegacyWecomTagGroup({ group_name: input.name, first_tag_name: input.firstTag, ...metadata }, opt))); return tagGroupPageDto(result.group); } const result = obj(await call(updateLegacyWecomTagGroupPatch(input.id, { group_name: input.name, ...metadata }, opt))); return tagGroupPageDto(result.group); }
export async function saveTagDto(input: { id?: number; groupId: number; name: string }): Promise<WecomTag> { const metadata = writeMeta(); const opt = tagWriteOptions(metadata); if (input.id != null) { const result = obj(await call(updateLegacyWecomTagPatch(input.id, { tag_name: input.name, ...metadata }, opt))); return tagPageDto(result.tag); } const group = obj(await call(getLegacyWecomTagGroup(input.groupId, opt))).group; if (!group) throw new Error('标签组不存在或已被删除'); const result = obj(await call(createLegacyWecomTag({ group_id: input.groupId, group_name: text(obj(group).name), tag_name: input.name, ...metadata }, opt))); return tagPageDto(result.tag); }
export async function archiveTagDto(tagId: number): Promise<void> { const metadata = writeMeta(); await call(archiveLegacyWecomTag(tagId, metadata, tagWriteOptions(metadata))); }
export async function queueTagSyncDto(): Promise<unknown> { const metadata = writeMeta(); return call(queueLegacyWecomTagSync(metadata, tagWriteOptions(metadata))); }
...
export async function readAdminRows(page?: string, customerList?: CustomerListQuery, miniProgramList?: MiniProgramListQuery): Promise<AdminDbWithMiniProgramList> {
  ...
  const responses = await Promise.all([
    ...
    needs('channels', 'channelForm', 'questionnaireOps') ? call(listLegacyChannels({ limit: 50, include_archived: true }, opt)) : skip,
...
export async function readAdminPage(context: AdminReadContext = {}): Promise<AdminDbWithMiniProgramList> {
  ...
  if (!id) return db;
  if (context.page === 'questionnaireDetail') { const [detail, results, submissions] = await Promise.all([call(getLegacyQuestionnaire(numeric, opt)), call(getLegacyQuestionnaireResults(numeric, opt)), call(listLegacyQuestionnaireSubmissions(numeric, undefined, opt))]); const q = obj(detail).questionnaire || detail; db.rows.questionnaires = [questionnairePageDto(q as LegacyQuestionnaire)]; db.rows.qSubs = list(submissions, 'items', 'submissions').map((x) => ({ time: text(obj(x).submitted_at), uid: text(obj(x).customer_id), by: text(obj(x).customer_name), score: text(obj(x).score), tags: list(obj(x).tags).map(String) })); void results; }
  ...
  if (context.page === 'orderDetail') { const order = db.rows.orders.find((item) => item.no === id); if (!order) throw new Error('订单不存在或不在当前白名单数据中'); db.rows.orders = [order]; db.rows.orderKv = [{ k: '订单号', v: order.no, mono: true }, { k: '商品', v: order.product || '—', mono: false }, { k: '客户引用', v: order.uid || '—', mono: true }, { k: '金额', v: order.amount, mono: false }, { k: '币种', v: order.pay || '—', mono: false }, { k: '创建时间', v: order.time || '—', mono: false }]; db.rows.orderEvents = []; }
  ...
  if (context.page === 'spProductForm') { const [detail, members] = await Promise.all([call(getServicePeriodProduct(numeric, opt)), call(listServicePeriodMembers(numeric, undefined, opt))]); db.rows.spProducts = [serviceProductPageDto(detail)]; db.rows.orderKv = list(members, 'items').map((x) => ({ k: text(obj(x).name, text(obj(x).member_ref)), v: text(obj(x).status, text(obj(x).state)), mono: false })); }
  ...
  if (context.page === 'audienceEdit') { const [detail, configResult, memberResult] = await Promise.all([call(getAIAudiencePackage(numeric, opt)), call(getAIAudienceConfigurationVersion(numeric, opt)), call(listAIAudiencePackageMembers(numeric, { limit: 200, offset: 0 }, opt))]); const pkg = audiencePackagePageDto(obj(detail).package); const configVersion = obj(obj(configResult).configuration); pkg.configurationVersion = Number(configVersion.version || 0) || undefined; db.audiencePackages = [pkg]; db.audienceMembers[numeric] = list(memberResult, 'items').map((item) => ({ name: text(obj(item).nickname), external_userid: text(obj(item).external_userid), joinedAt: text(obj(item).entered_at) })); }
