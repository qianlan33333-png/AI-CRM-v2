...
func TestLoadManifestFailsClosed(t *testing.T) {
	...
	for name, spec := range map[string][2]string{
		...
		"contacts": {"id", "updated_at+id"}, "crm_user_identity_conflicts": {"id", "updated_at+id"}, "external_contact_bindings": {"id", "updated_at+id"}, "people": {"id", "updated_at+id"}, "wecom_external_contact_follow_users": {"id", "updated_at+id"},
	} {
		m.Tables = append(m.Tables, Table{Name: name, PrimaryKey: spec[0], Watermark: spec[1], SchemaDigest: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", Mode: "full"})
	}
