...
// Store is Automation-owned. The Events append remains behind eventport.Appender.
...
func (s *Service) appendEvent(ctx context.Context, operation string, item automationport.Agent, actor int64, key string, now time.Time) error {
	eventType := map[string]string{"create": "automation.agent.created", "update": "automation.agent.updated", "copy": "automation.agent.copied", "publish": "automation.agent.published", "set_status": "automation.agent.status_changed", "fixed_content": "automation.agent.fixed_content_updated"}[operation]
	if eventType == "" {
		return ErrAgentUnavailable
	}
	...
	_, err = s.events.Append(ctx, eventport.Event{Type: eventType, Payload: payload, OccurredAt: now, IdempotencyKey: "automation.agent." + operation + ":" + hex.EncodeToString(digest[:])})
	return err
}
