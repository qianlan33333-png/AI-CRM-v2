...
func TestSetupWizardSaveRejectsMixedAuditStateWithoutPartialCommit(t *testing.T) {
	...
	repo.nextAuditID = 1
	repo.audits[commands[0].RequestID] = configstore.Audit{
...
func TestSetupWizardBatchAndManagerSetSerializeOnSharedAdvisoryLock(t *testing.T) {
	base := &wizardRepository{settings: map[configport.Key]configport.Setting{}, audits: map[string]configstore.Audit{}}
...
func newSetupWizardServiceForTest(t *testing.T, configured SetupWizardSecretConfigured) (*SetupWizardService, *wizardUoW, *wizardRepository, *wizardAppender) {
	t.Helper()
	repo := &wizardRepository{settings: map[configport.Key]configport.Setting{}, audits: map[string]configstore.Audit{}}
	events := &wizardAppender{}
...
func (uow *wizardUoW) Within(ctx context.Context, callback func(context.Context) error) error {
	...
	events := append([]eventport.Event(nil), uow.events.records...)
	err := callback(ctx)
	if err != nil {
...
type wizardRepository struct {
	...
	audits          map[string]configstore.Audit
	nextAuditID     int64
	upsertCalls     int
...
func (repository *wizardRepository) InsertAudit(_ context.Context, oldValue []byte, command configport.SetCommand, canonical []byte, updatedAt time.Time) (configstore.Audit, bool, error) {
	if _, exists := repository.audits[command.RequestID]; exists {
		return configstore.Audit{}, false, nil
	}
	repository.nextAuditID++
	audit := configstore.Audit{
...
func (repository *wizardRepository) GetAuditByRequestID(_ context.Context, requestID string) (configstore.Audit, error) {
	audit, found := repository.audits[requestID]
	if !found {
		return configstore.Audit{}, errors.New("audit not found")
	}
...
func (appender *wizardAppender) Append(_ context.Context, event eventport.Event) (eventport.EventID, error) {
	appender.calls++
	appender.records = append(appender.records, event)
	if appender.failOnCall == appender.calls {
		return 0, appender.err
	}
	return eventport.EventID(appender.calls), nil
}
...
func cloneWizardAudits(source map[string]configstore.Audit) map[string]configstore.Audit {
	clone := make(map[string]configstore.Audit, len(source))
