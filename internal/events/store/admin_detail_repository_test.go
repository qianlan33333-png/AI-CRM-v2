...
func TestAdminDetailRepositoryRequiresReadOnlyPointQueries(t *testing.T) {
	repository := NewAdminDetailRepository(nil)
	if _, err := repository.Read(context.Background(), 1); err == nil {
		t.Fatal("nil database unexpectedly succeeded")
	}
	contents, err := os.ReadFile("queries/admin_detail.sql")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, forbidden := range []string{"payload", "customer_id", "idempotency_key", "lease_owner", "lease_expires_at", "last_error_code", "river_job_id", "raw_error", "FOR UPDATE", "INSERT", "UPDATE", "DELETE"} {
		if strings.Contains(strings.ToUpper(query), strings.ToUpper(forbidden)) {
			t.Fatalf("query contains forbidden token %q", forbidden)
		}
	}
	contents, err = os.ReadFile("admin_detail_repository.go")
	if err != nil {
		t.Fatal(err)
	}
