...
type settingsProjectionRepository interface {
	ListAppSettings(context.Context) ([]configstore.ProjectionSetting, error)
	ListAppSettingsAudit(context.Context) ([]configstore.ProjectionAudit, error)
}
...
func (service *SettingsCompatibilityService) List(ctx context.Context, input SettingsListInput) (projection SettingsProjection, err error) {
	...
	var stored []configstore.ProjectionSetting
	var audits []configstore.ProjectionAudit
	...
	byKey := make(map[configport.Key]configstore.ProjectionSetting, len(stored))
