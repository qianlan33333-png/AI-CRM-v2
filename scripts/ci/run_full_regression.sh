...
go test -count=1 ./internal/contact/store -run '^TestInvalidSourceHistoryPostgresRoundTripRollback$' -contact-invalid-source-history-postgres-dsn="$database_url"
go test -count=1 ./internal/media/store -run '^TestInvalidAssetHistoryPostgresRoundTripRollback$' -media-invalid-source-history-postgres-dsn="$database_url"
go test -count=1 ./internal/radar/store -run '^TestInvalidRadarLinkHistoryPostgresRoundTripRollback$' -radar-invalid-source-history-postgres-dsn="$database_url"
...
go test -count=1 ./internal/contact/store -run '^TestDeferredIdentityHistoryPostgresRoundTripRollback$' -deferred-identity-history-postgres-dsn="$database_url"
