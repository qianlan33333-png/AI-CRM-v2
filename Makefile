...
.PHONY: p4-f01a-survey-acceptance p4-f01ab-survey-acceptance p4-survey-identity-external-push-acceptance p4-ai-audience-local-configuration-acceptance p4-ai-audience-operation-member-acceptance p4-ai-audience-inbound-webhook-acceptance p4-group-ops-acceptance p4-group-ops-runtime-acceptance
...
.PHONY: p4-commerce-external-push-acceptance
.PHONY: p4-ci01-external-read-acceptance
...
query-plan-gate:
	@/usr/bin/env -u BASH_ENV -u ENV GOWORK=off GOTOOLCHAIN=local GOFLAGS=-mod=readonly $(GO) -C tools run ./query-plan-gate -root .. -base "$$QUERY_PLAN_BASE_SHA" -head "$$QUERY_PLAN_HEAD_SHA" -database-url "$$QUERY_PLAN_TEST_DATABASE_URL" -all="$${QUERY_PLAN_ALL:-false}"
...
p4-radar-local-tracking-acceptance:
	@test -n "$${P4_RADAR_TRACKING_TEST_DATABASE_URL:-}" || { echo "P4_RADAR_TRACKING_TEST_DATABASE_URL is required" >&2; exit 2; }
	@GO="$(GO)" TOOLS_MOD="$(TOOLS_MOD)" acceptance/radar/tracking_pg16.sh

p4-commerce-external-push-acceptance:
	@test -n "$${P4EXTERNALPUSH_TEST_DATABASE_URL:-}" || { echo "P4EXTERNALPUSH_TEST_DATABASE_URL is required" >&2; exit 2; }
	@GO="$(GO)" TOOLS_MOD="$(TOOLS_MOD)" acceptance/product/external_push_00087_pg16.sh
	@/usr/bin/env -u BASH_ENV -u ENV GOWORK=off GOTOOLCHAIN=local GOFLAGS=-mod=readonly $(GO) test -race -count=1 -timeout=180s ./internal/product/... ./cmd/aicrm

p4-ci01-external-read-acceptance:
	@test -n "$${P4CI01_TEST_DATABASE_URL:-}" || { echo "P4CI01_TEST_DATABASE_URL is required" >&2; exit 2; }
	@/usr/bin/env -u BASH_ENV -u ENV GOWORK=off GOTOOLCHAIN=local GOFLAGS=-mod=readonly $(GO) test -race -count=1 -timeout=60s -run '^TestCI01UnionIDLookupExcludesUnverifiedIdentity$$' ./acceptance/identity -args -database-url "$$P4CI01_TEST_DATABASE_URL"
...
p4-group-ops-runtime-acceptance:
	@test -n "$${P4GROUP_OPS_TEST_DATABASE_URL:-}" || { echo "P4GROUP_OPS_TEST_DATABASE_URL is required" >&2; exit 2; }
	@GO="$(GO)" TOOLS_MOD="$(TOOLS_MOD)" /usr/bin/env bash acceptance/groupops/group_ops_runtime_migration_compatibility.sh
...
p4-ai-audience-local-configuration-acceptance:
	@test -n "$${P4AIAUDIENCE_TEST_DATABASE_URL:-}" || { echo "P4AIAUDIENCE_TEST_DATABASE_URL is required" >&2; exit 2; }
	@GO="$(GO)" TOOLS_MOD="$(TOOLS_MOD)" /usr/bin/env bash acceptance/segment/ai_audience_local_configuration_pg16.sh

p4-ai-audience-operation-member-acceptance:
	@test -n "$${P4AIAUDIENCEOPMEMBER_TEST_DATABASE_URL:-}" || { echo "P4AIAUDIENCEOPMEMBER_TEST_DATABASE_URL is required" >&2; exit 2; }
	@GO="$(GO)" TOOLS_MOD="$(TOOLS_MOD)" /usr/bin/env bash acceptance/segment/ai_audience_operation_member_pg16.sh

p4-ai-audience-inbound-webhook-acceptance:
	@test -n "$${P4AIAUDIENCEINBOUNDWEBHOOK_TEST_DATABASE_URL:-}" || { echo "P4AIAUDIENCEINBOUNDWEBHOOK_TEST_DATABASE_URL is required" >&2; exit 2; }
	@GO="$(GO)" TOOLS_MOD="$(TOOLS_MOD)" /usr/bin/env bash acceptance/segment/ai_audience_inbound_webhook_pg16.sh
