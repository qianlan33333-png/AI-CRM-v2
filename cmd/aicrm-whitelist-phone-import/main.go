...
func readSource(input io.Reader) ([]sourceRow, string, error) {
	...
	for _, record := range records[1:] {
		...
		seen[unionID] = struct{}{}
		verified := record[2] == "t" || record[2] == "true"
...
func classify(ctx context.Context, tx pgx.Tx, rows []sourceRow, digest string) ([]classifiedRow, report, error) {
	...
	phoneCustomers := map[string]map[int64]struct{}{}
	for _, row := range rows {
		...
		default:
			...
			if err := tx.QueryRow(ctx, `SELECT customer_id FROM public.source_subject_refs WHERE source_system='aicrm_v2_frozen' AND source_entity='unionid' AND reference_digest=$1`, reference[:]).Scan(&item.customerID); errors.Is(err, pgx.ErrNoRows) {
				...
				phoneCustomers[item.phone][item.customerID] = struct{}{}
			}
		}
	...
	for index := range classified {
		...
		if item.disposition == migrate && len(phoneCustomers[item.phone]) != 1 {
			item.disposition, item.reason = rejectConflict, "verified phone maps to multiple V2 customers"
		}
...
RETURNING id,(xmax=0)`, row.customerID, row.phone, phoneSource, fingerprint).Scan(&identityID, &found)
