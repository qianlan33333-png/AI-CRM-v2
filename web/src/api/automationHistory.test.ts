...
export async function runAutomationHistoryAdapterTests(): Promise<void> {
  ...
  const config = { id: 2, source_id: 102, source_key_digest: digest, source_payload_digest: digest, agent_code: 'agent', display_name: '配置', scenario_code: 'scenario', original_enabled: true, draft_version: -2, published_version: 0, published_at: at, last_modified_at: at, last_modified_source: 'legacy', submitted_for_publish: false, submitted_at: at, created_at: at, updated_at: at, actors_digest: digest, config_digest: digest };
  ...
  try {
    assert((await readAutomationHistoryConfigs()).items[0].draft_version === -2 && (await readAutomationHistoryAgents()).items[0].workflow_source_id === 0, 'historical negative and zero values changed');
    calls.length = 0;
