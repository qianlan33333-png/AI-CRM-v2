...
[[ "$GOOSE_DRIVER" = postgres && "$*" = *"-dir ${AICRM_FINAL_GOOSE_TEST_MIGRATIONS:?} up-to 144" ]]
EOF
chmod 700 "$goose"
...
AICRM_FINAL_GOOSE_TEST_LOG="$log" AICRM_FINAL_GOOSE_TEST_MIGRATIONS="$root/migrations" \
  "$goose_runner" --from=135 --to=144 --runtime-env-file="$env_file"
[[ "$(grep -c '^args=' "$log")" = 1 ]] || fail 'Goose was not invoked exactly once'
grep -Fq "args=-dir $root/migrations up-to 144" "$log" || fail 'Goose bounds or migration directory changed'
...
grep -Fq 'only one bounded Goose migration from 135 to 144 is accepted' "$fixture/bounds.log" || fail 'wrong bound rejection changed'
printf '0' >>"$goose"
if AICRM_FINAL_GOOSE_TEST_LOG="$log" "$goose_runner" --from=135 --to=144 --runtime-env-file="$env_file" >"$fixture/seal.log" 2>&1; then fail 'modified bundled Goose was accepted'; fi
