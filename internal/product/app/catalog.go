...
func (s *Service) Create(ctx context.Context, command productport.CreateCommand) (productport.Product, error) {
	...
	err = s.uow.Within(ctx, func(tx context.Context) error {
		...
		if _, e = s.events.Append(tx, eventport.Event{Type: eventport.EvProductCreated, Payload: eventPayload, OccurredAt: now, IdempotencyKey: "product.create:" + hex.EncodeToString(eventDigest[:])}); e != nil {
			return e
		}
