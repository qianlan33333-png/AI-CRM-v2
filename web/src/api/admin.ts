...
export async function readAdminPage(context: AdminReadContext = {}): Promise<AdminDb> {
  ...
  if (context.page === 'spProductForm') { const [detail, members, access, schema, views, share] = await Promise.all([call(getServicePeriodProduct(numeric, opt)), call(listServicePeriodMembers(numeric, undefined, opt)), call(getServicePeriodMemberGridAccess(numeric, opt)), call(getServicePeriodMemberGridSchema(numeric, opt)), call(listServicePeriodMemberViews(numeric, opt)), call(getServicePeriodMemberGridShareSettings(numeric, opt))]); const shareState = obj(share); db.rows.spProducts = [serviceProductPageDto(detail)]; db.rows.orderKv = [...list(members, 'items').map((x) => ({ k: text(obj(x).name), v: text(obj(x).status), mono: false })), { k: 'member-grid', v: text(obj(schema).version), mono: false }, { k: 'views', v: String(list(views, 'items').length), mono: false }, { k: 'share-supported', v: String(shareState.external_share_supported === true), mono: false }, { k: 'share-enabled', v: String(shareState.external_share_enabled === true), mono: false }, { k: 'share-version', v: String(Number(shareState.external_share_version || 0)), mono: false }, { k: 'access', v: text(obj(access).role), mono: false }]; }
