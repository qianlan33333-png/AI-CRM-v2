...
const productPanels = [...document.querySelectorAll('[data-product-panel-content]')];
if (productPanels.length !== 5 || document.querySelector('#product-sale')?.style.display === 'none' || productPanels.filter((panel) => panel.id !== 'product-sale').some((panel) => panel.style.display !== 'none')) throw new Error('普通商品未按 V1 只显示当前配置面板');
document.querySelector('[data-product-panel="media"]')?.click();
await new Promise((resolve) => setTimeout(resolve, 20));
if (document.querySelector('#product-sale')?.style.display !== 'none' || document.querySelector('#product-media')?.style.display === 'none') throw new Error('普通商品配置维度切换未保持 V1 单面板逻辑');
document.querySelector('[data-product-panel="sale"]')?.click();
await new Promise((resolve) => setTimeout(resolve, 20));
...
document.querySelector('#pfPrice').value = '219.00';
document.querySelector('#pfCode').value = 'P-21-NEW';
document.querySelector('#pfPrice').dispatchEvent(new dom.window.Event('input', { bubbles: true }));
document.querySelector('#pfCode').dispatchEvent(new dom.window.Event('input', { bubbles: true }));
if (document.querySelector('#pfSummaryPrice')?.textContent !== '219.00' || document.querySelector('#pfSummaryCode')?.textContent !== 'P-21-NEW') throw new Error('普通商品顶部摘要未随表单输入实时更新');
...
const servicePanels = [...spDocument.querySelectorAll('[data-service-period-panel-content]')];
if (servicePanels.length !== 4 || spDocument.querySelector('#sp-sale')?.style.display === 'none' || servicePanels.filter((panel) => panel.id !== 'sp-sale').some((panel) => panel.style.display !== 'none')) throw new Error('周期商品未按 V1 只显示当前配置面板');
spDocument.querySelector('[data-service-period-panel="push"]')?.click();
await new Promise((resolve) => setTimeout(resolve, 20));
if (spDocument.querySelector('#sp-sale')?.style.display !== 'none' || spDocument.querySelector('#sp-push')?.style.display === 'none') throw new Error('周期商品配置维度切换未保持 V1 单面板逻辑');
spDocument.querySelector('[data-service-period-panel="sale"]')?.click();
await new Promise((resolve) => setTimeout(resolve, 20));
...
spDocument.querySelector('#spfPrice').value = '299.00';
spDocument.querySelector('#spfCode').value = 'SP-8-NEW';
spDocument.querySelector('#spfPrice').dispatchEvent(new spDom.window.Event('input', { bubbles: true }));
spDocument.querySelector('#spfCode').dispatchEvent(new spDom.window.Event('input', { bubbles: true }));
if (spDocument.querySelector('#spfSummaryPrice')?.textContent !== '299.00' || spDocument.querySelector('#spfSummaryCode')?.textContent !== 'SP-8-NEW') throw new Error('周期商品顶部摘要未随表单输入实时更新');
