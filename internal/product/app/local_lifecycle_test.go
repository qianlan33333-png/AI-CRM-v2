...
func TestLocalProductLifecycleCopyRetainsTypedBodyAndImagesButResetsDraft(t *testing.T) {
	...
	if store.createCalls != 1 || len(events.events) != 1 || events.events[0].Type != eventport.EvProductCreated {
...
func TestLocalProductLifecycleDeleteIsDraftOnlyAndReferenceSafe(t *testing.T) {
	...
	if _, ok := store.products[draft.ID]; ok || len(events.events) != 1 || events.events[0].Type != eventport.EvProductUpdated {
...
func (events *localProductLifecycleTestEvents) Append(_ context.Context, event eventport.Event) (eventport.EventID, error) {
