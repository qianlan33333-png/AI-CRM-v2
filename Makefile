...
p4-automation-rules-runtime-acceptance:
	...
	@/usr/bin/env -u BASH_ENV -u ENV GOWORK=off GOTOOLCHAIN=local GOFLAGS=-mod=readonly $(GO) test -race -count=1 -timeout=180s -run '^TestA01TagRuleEnrollmentSnapshotsActionsAndNeverCallsAProvider$$' ./acceptance/automation -args -database-url "$$P4AUTOMATIONRULES_TEST_DATABASE_URL"
