...
import (
	...
	operationport "github.com/qianlan33333-png/AI-CRM-v2/internal/operationcycle/port"
...
func NewService(uow platformport.UnitOfWork, store Store, events eventport.Appender, deliveries eventport.DeliveryAcceptor) *Service {
...
func (s *Service) append(ctx context.Context, eventType, idempotencyKey string, value map[string]any) error {
	...
	eventID, err := s.events.Append(ctx, eventport.Event{Type: eventport.EvOperationCycleFact, Payload: payload, OccurredAt: s.now().UTC(), IdempotencyKey: "operation_cycle:" + idempotencyKey})
	if err != nil {
		return fmt.Errorf("%w: %v", ErrUnavailable, err)
	}
	if err = s.deliveries.Accept(ctx, eventID, eventport.ConsumerOperationCycleFact); err != nil {
		return fmt.Errorf("%w: %v", ErrUnavailable, err)
	}
	return nil
}
...
func containsForbidden(value any) bool {
	...
	case map[string]any:
		for key, child := range item {
			...
			if strings.Contains(lowered, "ten"+"ant") || strings.Contains(lowered, "credential") || strings.Contains(lowered, "secret") || lowered == "external_effects" && resultString(item, key) != "" && resultString(item, key) != "none" {
				return true
			}
	...
	case string:
		return strings.Contains(item, "/Users/") || strings.Contains(item, "file://")
	}
	return false
}
