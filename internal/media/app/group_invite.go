...
func (service *GroupInviteService) mutateAt(ctx context.Context, operation, businessKey string, actor int64, key string, seed mediaport.GroupInvite, patch mediaport.GroupInvitePatch, now time.Time) (mediaport.GroupInvite, error) {
	...
	err = service.uow.Within(ctx, func(tx context.Context) error {
		...
		if _, innerErr = service.events.Append(tx, eventport.Event{Type: groupInviteEventType(operation), Payload: eventPayload, OccurredAt: now,
...
func groupInviteEventType(operation string) string {
	switch operation {
	case "create":
		return eventport.EvMediaGroupInviteCreated
	case "update":
		return eventport.EvMediaGroupInviteUpdated
	case "archive":
		return eventport.EvMediaGroupInviteArchived
	default:
		return ""
	}
}
