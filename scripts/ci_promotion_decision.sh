...
is_framework_risk_path() {
  ...
  if [[ "$candidate_path" = "scripts/generated-sources.sha256" ]]; then
    return 1
  fi
  if [[ "$candidate_path" = migrations/* && ! "$candidate_path" =~ ^migrations/[0-9]{5}_[A-Za-z0-9_]+\.sql$ ]]; then
    return 0
  fi
  if [[ "$candidate_path" =~ ^internal/(adminops|auth|config|gateway)/ ]] ||
    [[ "$candidate_path" =~ ^cmd/aicrm/legacy_(admin|auth|config)[A-Za-z0-9._-]*\.go$ ]]; then
    return 0
  fi
  [[ "$candidate_path" =~ ^(\.github/|scripts/|tools/|acceptance/fixtures/|internal/platform/|docs/(adr|architecture|governance|plans|spec|execution/slice-ledger\.yml|execution/implementation-plan\.md|evidence/phases/)|Makefile$|go\.(mod|sum)$|package(-lock)?\.json$|sqlc\.ya?ml$|api/oapi-codegen[^/]*\.ya?ml$|\.gitleaks\.toml$|SECURITY\.md$|cmd/aicrm/main\.go$|cmd/aicrm/components\.go$) ]]
}

is_domain_artifact_path() {
  local candidate_path="$1"
  [[ "$candidate_path" =~ ^migrations/[0-9]{5}_[A-Za-z0-9_]+\.sql$ ]] ||
    [[ "$candidate_path" = "api/openapi.yaml" ]] ||
    [[ "$candidate_path" =~ ^(internal/api/(candidate/)?generated|web/src/api/generated)/[A-Za-z0-9._/-]+$ ]] ||
    [[ "$candidate_path" =~ ^internal/(adminops|ai|auth|automation|config|contact|coupon|events|gateway|identity|media|operationcycle|ops|order|outbound|product|pushcenter|segment|stats|survey|wecom)/[A-Za-z0-9._/-]+$ ]] ||
    [[ "$candidate_path" =~ ^acceptance/(adminops|ai|auth|automation|config|contact|coupon|events|gateway|identity|media|operationcycle|ops|order|outbound|product|pushcenter|segment|stats|survey|wecom)/[A-Za-z0-9._/-]+$ ]] ||
    [[ "$candidate_path" =~ ^cmd/aicrm/legacy_[A-Za-z0-9._-]+\.go$ ]] ||
    [[ "$candidate_path" =~ ^docs/(feature-matrix\.csv|migration-mapping\.jsonl|api-mapping\.jsonl|execution/slices/[A-Za-z0-9._-]+\.md|evidence/slices/[A-Za-z0-9._-]+\.md|ci/(go-acceptance-manifest|repo-contract-fingerprints)\.tsv)$ ]] ||
...
[[ "$(jq -r '.sha // empty' <<<"$current_commit")" = "$commit_sha" ]] || fallback "current-commit-sha-mismatch"
...
[[ "$(jq -r '.sha // empty' <<<"$head_commit")" = "$head_sha" ]] || fallback "pr-head-sha-mismatch"
...
for context in "${required_checks[@]}"; do
  ...
  started_at="$(jq -r '.[0].started_at // empty' <<<"$matching")"
  completed_at="$(jq -r '.[0].completed_at // empty' <<<"$matching")"
  [[ -n "$started_at" && -n "$completed_at" && "$started_at" > "$head_time" && "$completed_at" > "$started_at" ]] ||
    fallback "stale-required-check"
done
