...
grep -Fq '"table_count":2' "$failure_log"
grep -Fq '"row_count":4' "$failure_log"
GOWORK=off GOTOOLCHAIN=local GOFLAGS=-mod=readonly \
...
[[ "$(psql "$database_url" -X -q -At -c "SELECT count(*)||'|'||count(DISTINCT source_key_digest) FROM v1_archive_records WHERE run_id='v1-archive-acceptance'")" = "4|4" ]]
