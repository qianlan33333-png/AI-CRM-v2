...
[[ "$baseline_admin_count" =~ ^[1-9][0-9]*$ && "$baseline_admin_hash" =~ ^[0-9a-f]{32}$ ]]
...
/usr/bin/env -u BASH_ENV -u ENV GOWORK=off GOTOOLCHAIN=local GOFLAGS=-mod=readonly \
  ...
  -run '^TestI01A(Create|Event|S200K|Storage)' ./acceptance/product -args -database-url "$database_url"
...
printf 'P4-I01A migration compatibility: PASS (28/29/28/29, pre-existing Event/Auth/session history preserved)\n'
