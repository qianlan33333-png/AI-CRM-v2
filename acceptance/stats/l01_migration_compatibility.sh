...
base_database_url="$P4W0L01_STATS_TEST_DATABASE_URL"
temporary_database="aicrm_test_p4_w0_l01_stats"
database_url="${base_database_url/aicrm_test/$temporary_database}"

cleanup() {
  psql "$base_database_url" -X -q -v ON_ERROR_STOP=1 -c \
    "DROP DATABASE IF EXISTS $temporary_database WITH (FORCE)" >/dev/null
}
trap cleanup EXIT
cleanup
psql "$base_database_url" -X -q -v ON_ERROR_STOP=1 -c \
  "CREATE DATABASE $temporary_database" >/dev/null
...
[[ "$daily_table" = "1" && "$receipt_table" = "1" && "$daily_index" = "1" ]]
...
[[ "$receipt_count" = "1" && "$projection_value" = "1" ]]
[[ "$delivery_status" = "completed" && "$automation_receipt" = "1" && "$job_count" = "1" ]]

printf 'P4-W0-L01 migration compatibility: PASS (history preserved across rollback and latest upgrade)\n'
