...
func TestSetupWizardPOSTUsesPrincipalIdempotencyAndStrictLocalResult(t *testing.T) {
	...
	response := httptest.NewRecorder()
	handler.ServeHTTP(response, request)
	if response.Code != http.StatusOK || application.saveCalls != 1 || !strings.Contains(response.Body.String(), `"external":false`) || !strings.Contains(response.Body.String(), `"local_only":true`) || !strings.Contains(response.Body.String(), `"runtime_applied":false`) {
		t.Fatalf("POST status/calls/body=%d/%d/%s", response.Code, application.saveCalls, response.Body.String())
	}
...
func TestSetupWizardPOSTRejectsRouteTokenAndSecretWithoutLeakingIt(t *testing.T) {
