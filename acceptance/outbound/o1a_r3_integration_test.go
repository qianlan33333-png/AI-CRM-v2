...
var outboundO6B2MigrationProof = flag.Bool("o6b2-migration-proof", false, "run isolated O6B2 migration compatibility proof")
...
func TestOutboundStorageCatalogWaterlineAndIdentity(t *testing.T) {
	...
	var waterline int
	if err := pool.QueryRow(ctx, `SELECT max(version_id) FROM goose_db_version WHERE is_applied`).Scan(&waterline); err != nil || waterline < 37 {
		t.Fatalf("migration waterline=%d err=%v, want at least 37", waterline, err)
	}
...
func openOutboundPool(t *testing.T) *pgxpool.Pool {
	...
	if err := validateOutboundDatabaseURL(*outboundDatabaseURL, *outboundO6ARealRiver, *outboundO6B1MigrationProof, *outboundO6B2MigrationProof); err != nil {
...
func validateOutboundDatabaseURL(databaseURL string, allowO6ATemporary, allowO6B1Temporary, allowO6B2Temporary bool) error {
	...
	if allowO6B1Temporary && acceptancefixtures.ValidateDatabaseURLForDatabase(databaseURL, acceptancefixtures.O6B1CancelDatabaseName) == nil {
		return nil
	}
	if allowO6B2Temporary {
		return acceptancefixtures.ValidateDatabaseURLForDatabase(databaseURL, acceptancefixtures.O6B2ManualRetryDatabaseName)
	}
	return err
}
...
func TestOutboundDatabaseURLValidationIsClosed(t *testing.T) {
	...
	o6b2TemporaryURL := "postgres://postgres:postgres@127.0.0.1:5432/" + acceptancefixtures.O6B2ManualRetryDatabaseName + "?sslmode=disable"
	if err := validateOutboundDatabaseURL(acceptancefixtures.DefaultDatabaseURL, true, true, true); err != nil {
		t.Fatalf("shared O6A database rejected: %v", err)
	}
	if err := validateOutboundDatabaseURL(o6aTemporaryURL, true, false, false); err != nil {
		t.Fatalf("dedicated O6A database rejected: %v", err)
	}
	if err := validateOutboundDatabaseURL(o6aTemporaryURL, false, false, false); !errors.Is(err, acceptancefixtures.ErrUnsafeDatabaseURL) {
		t.Fatalf("dedicated O6A database accepted outside O6A: %v", err)
	}
	if err := validateOutboundDatabaseURL(o6b1TemporaryURL, false, true, false); err != nil {
		t.Fatalf("dedicated O6B1 database rejected: %v", err)
	}
	if err := validateOutboundDatabaseURL(o6b1TemporaryURL, false, false, false); !errors.Is(err, acceptancefixtures.ErrUnsafeDatabaseURL) {
		t.Fatalf("dedicated O6B1 database accepted outside O6B1: %v", err)
	}
	if err := validateOutboundDatabaseURL(o6b2TemporaryURL, false, false, true); err != nil {
		t.Fatalf("dedicated O6B2 database rejected: %v", err)
	}
	if err := validateOutboundDatabaseURL(o6b2TemporaryURL, false, false, false); !errors.Is(err, acceptancefixtures.ErrUnsafeDatabaseURL) {
		t.Fatalf("dedicated O6B2 database accepted outside O6B2: %v", err)
	}
	if err := validateOutboundDatabaseURL("postgres://postgres:postgres@127.0.0.1:5432/aicrm_test_other?sslmode=disable", true, true, true); !errors.Is(err, acceptancefixtures.ErrUnsafeDatabaseURL) {
