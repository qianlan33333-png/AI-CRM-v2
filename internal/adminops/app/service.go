...
type repository interface {
	CreateCredential(context.Context, adminopsport.Credential) (adminopsport.Credential, error)
	...
	UpdateCredential(context.Context, adminopsport.Credential) (adminopsport.Credential, error)
	UpsertCategory(context.Context, adminopsport.Category) (adminopsport.Category, error)
	GetCategory(context.Context, string) (adminopsport.Category, error)
	ListCategories(context.Context) ([]adminopsport.Category, error)
	CreateRelease(context.Context, adminopsport.Release) (adminopsport.Release, error)
	GetRelease(context.Context, int64) (adminopsport.Release, error)
	ListReleases(context.Context, int32) ([]adminopsport.Release, error)
	ValidateRelease(context.Context, int64, time.Time) (adminopsport.Release, error)
	PublishRelease(context.Context, int64, string, string, time.Time) (adminopsport.Release, error)
	RollbackRelease(context.Context, int64, string, time.Time) (adminopsport.Release, error)
	CreateJob(context.Context, adminopsport.Job) (adminopsport.Job, error)
	GetJob(context.Context, string) (adminopsport.Job, error)
	ListJobs(context.Context, string, string, int32) ([]adminopsport.Job, error)
	TransitionJob(context.Context, adminopsport.Job) (adminopsport.Job, error)
	GetNotification(context.Context) (adminopsdb.AdminOpsNotificationSetting, error)
	UpsertNotification(context.Context, bool, string, string, string, string, time.Time) (adminopsdb.AdminOpsNotificationSetting, error)
	ReserveReceipt(context.Context, string, string, []byte, []byte, time.Time) (adminopsstore.Receipt, bool, error)
	CompleteReceipt(context.Context, int64, []byte, time.Time) (adminopsstore.Receipt, error)
}
...
func NewService(uow platformport.UnitOfWork, store repository) *Service {
...
func (service *Service) GetFeishuNotification(ctx context.Context) (NotificationSetting, error) {
	...
	err := service.uow.Within(ctx, func(tx context.Context) error {
		...
		if errors.Is(itemErr, adminopsstore.ErrNotFound) {
...
func validJobKind(value string) bool {
	switch value {
	case "archive_sync", "message_batch_ack", "feishu_webhook_validate", "feishu_hourly_report", "order_identity_repair":
		return true
	}
	return false
}
...
func notificationFromRow(row adminopsdb.AdminOpsNotificationSetting) NotificationSetting {
	return NotificationSetting{Enabled: row.Enabled, Channel: row.Channel, SecretRef: row.SecretRef, SecretMask: row.SecretMask, ValidationState: row.ValidationState, UpdatedAt: row.UpdatedAt.Time.UTC()}
}
...
func classify(err error) error {
	...
	if errors.Is(err, adminopsstore.ErrNotFound) {
		return adminopsstore.ErrNotFound
	}
