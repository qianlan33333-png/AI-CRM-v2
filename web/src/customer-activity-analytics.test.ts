...
import { generatedCustomerActivityAnalyticsTransport, loadCustomerActivityAnalytics, parseCustomerActivityAnalytics, type CustomerActivityAnalyticsTransport } from "./customer-activity-analytics";
...
describe("customer activity analytics contract", () => {
  it("uses the generated same-origin customer analytics endpoint", async () => {
    const fetchMock = vi.fn(async () => new Response(JSON.stringify(analytics), { status: 200 }));
    vi.stubGlobal("fetch", fetchMock);
    try {
      await generatedCustomerActivityAnalyticsTransport.get(41, { window_days: 30 }, { credentials: "same-origin" });
      expect(fetchMock).toHaveBeenCalledWith("/api/v1/customers/41/activity-analytics?window_days=30", { credentials: "same-origin", method: "GET" });
