// Package acceptancefixture creates Coupon-owned rows for acceptance scenarios.
...
func CreateProductTarget(ctx context.Context, db queryer, couponID, productID int64) error {
	if db == nil || couponID < 1 || productID < 1 {
		return fmt.Errorf("valid Coupon target fixture required")
	}
	if _, err := db.Exec(ctx, `INSERT INTO coupon_targets(coupon_id,position,target_ref,product_id) VALUES ($1,0,$2,$3)`, couponID, "standard_product:"+fmt.Sprint(productID), productID); err != nil {
