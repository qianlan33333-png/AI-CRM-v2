...
function traceID(value?: string): string | undefined {
  ...
  if (normalized.length > 200) throw new Error('trace_id 不能超过 200 个字符，已拒绝请求');
  return normalized;
}
...
export async function readPushTraceObservabilityDto(inputTraceID?: string): Promise<PushTraceObservability> {
  const requestedTraceID = traceID(inputTraceID);
  ...
  if (unavailable) return { traceID: requestedTraceID, degraded: true, message: unavailable, sections: [], counts: null };
