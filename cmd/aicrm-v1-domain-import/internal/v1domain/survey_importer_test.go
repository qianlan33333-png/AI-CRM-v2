...
func TestBuildSurveyAggregateResolvesSourceRelationsAndDropsRedactedToken(t *testing.T) {
	...
	questions := map[int64][]archivedValue[surveyQuestionJSON]{1: {{value: surveyQuestionJSON{
		ID: 2, QuestionnaireID: 1, Type: "single_choice", Title: "Q", SortOrder: 1, CreatedAt: stamp, UpdatedAt: stamp,
	}}}}
	options := map[int64][]archivedValue[surveyOptionJSON]{2: {{value: surveyOptionJSON{
		ID: 3, QuestionID: 2, OptionText: "A", TagCodes: json.RawMessage(`[]`), OtherMaximumLength: 80, SortOrder: 1, CreatedAt: stamp, UpdatedAt: stamp,
	}}}}
	...
	if aggregate.Questions[0].SortOrder != 0 || aggregate.Options[0].SortOrder != 0 || aggregate.Options[0].OtherMaxLength != 0 {
		t.Fatal("V1 positions and inactive option defaults were not converted")
	}
	if questions[1][0].value.SortOrder != 1 || options[2][0].value.OtherMaximumLength != 80 {
		t.Fatal("original archive values were modified")
	}
...
func TestBuildSurveyAggregatePreservesInvalidPositionsAndActiveOtherInput(t *testing.T) {
	questionnaire := archivedValue[surveyQuestionnaireJSON]{value: surveyQuestionnaireJSON{ID: 1}}
	questions := map[int64][]archivedValue[surveyQuestionJSON]{1: {
		{value: surveyQuestionJSON{ID: 2, QuestionnaireID: 1, SortOrder: 0}},
		{value: surveyQuestionJSON{ID: 3, QuestionnaireID: 1, SortOrder: 3}},
	}}
	options := map[int64][]archivedValue[surveyOptionJSON]{2: {
		{value: surveyOptionJSON{ID: 4, QuestionID: 2, SortOrder: 0, IsOther: true, OtherPlaceholder: "Other", OtherMaximumLength: 80}},
	}}
	aggregate, _ := buildSurveyAggregate(questionnaire, questions, options, nil, nil, map[int64]bool{}, map[int64]bool{}, map[int64]bool{}, map[int64]bool{})
	if aggregate.Questions[0].SortOrder != -1 || aggregate.Questions[1].SortOrder != 2 || aggregate.Options[0].SortOrder != -1 {
		t.Fatal("invalid V1 positions must not be silently renumbered")
	}
	if !aggregate.Options[0].IsOther || aggregate.Options[0].OtherPlaceholder != "Other" || aggregate.Options[0].OtherMaxLength != 80 {
		t.Fatal("active other-input configuration was lost")
	}
}
