...
| `00073_internal_event_safe_exports.sql` — EE01 Internal Event Safe Export (**local checkpoint; not in the frozen 10/73 denominator**) | Three Events-owned native operations freeze only local `event_log` / `event_deliveries` rows. No Radar, USER OPS, legacy Matrix, River, Outbound, Provider, or delivery claim is added. | global admin `admin.read`; create requires human session, CSRF and Idempotency-Key; actor-bound reads. Header/rows/audit/completed receipt share one UoW and are immutable. | `internal/events/*safe_export*` focused tests; `make p4-ee01-internal-event-safe-export-acceptance` creates and cleans a PG16.14 dedicated DB. | `none / NOT_EXECUTED`; no delivery enqueue, River job, outbound task or Provider call. |
