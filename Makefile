...
.PHONY: p4-f01a-survey-acceptance p4-f01ab-survey-acceptance p4-group-ops-acceptance p4-user-ops-acceptance
...
p4-user-ops-acceptance:
	@test -n "$${P4USEROPS_TEST_DATABASE_URL:-}" || { echo "P4USEROPS_TEST_DATABASE_URL is required" >&2; exit 2; }
	@$(GO) tool -modfile=$(TOOLS_MOD) goose -dir migrations postgres "$$P4USEROPS_TEST_DATABASE_URL" up
	@/usr/bin/env -u BASH_ENV -u ENV GOWORK=off GOTOOLCHAIN=local GOFLAGS=-mod=readonly $(GO) test -race -count=1 -timeout=180s ./internal/userops/... ./internal/contact/app ./internal/contact/store
	@/usr/bin/env -u BASH_ENV -u ENV GOWORK=off GOTOOLCHAIN=local GOFLAGS=-mod=readonly $(GO) test -race -count=1 -timeout=90s -run '^TestUserOps' ./cmd/aicrm
	@/usr/bin/env -u BASH_ENV -u ENV GOWORK=off GOTOOLCHAIN=local GOFLAGS=-mod=readonly $(GO) test -race -count=1 -timeout=90s -run '^TestP4UserOps' ./acceptance/contact -args -database-url "$$P4USEROPS_TEST_DATABASE_URL"
