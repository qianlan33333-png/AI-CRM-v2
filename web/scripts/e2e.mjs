    ...
    beforeParse(window) {
      ...
      window.wx = {
        ...
        invoke(method, payload, callback) {
          ...
          callback({ err_msg: method + ':ok', ...(method === 'getContext' ? { userId: 'staff-7' } : {}), ...(method === 'getCurExternalContact' ? { external_userid: 'ext-7' } : {}) });
        },
      };
...
console.log('sidebar/index.html');
{
  ...
  ok('无 external_userid 时保持 config → agentConfig → getContext(客服) → getCurExternalContact(客户) → bootstrap 安全顺序',
