...
// from the existing Product Store interface so current catalog/entitlement
// fakes and generated code remain untouched until the central integration
// lane materializes their SQL methods.
...
func NewLocalProductLifecycleService(uow platformport.UnitOfWork, store LocalProductLifecycleStore, events eventport.Appender) *LocalProductLifecycleService {
...
func (service *LocalProductLifecycleService) SetLocalProductEnabled(ctx context.Context, command productport.SetLocalProductEnabledCommand) (productport.LocalProduct, error) {
	...
	err = service.uow.Within(ctx, func(tx context.Context) error {
		...
		if eventErr := appendLocalProductLifecycleEvent(tx, service.events, action, result, 0, normalized.Actor, reservation, now, eventport.EvProductUpdated); eventErr != nil {
			return eventErr
		}
		return service.completeLocalProductSnapshot(tx, receipt.ID, result, now)
	})
...
func (service *LocalProductLifecycleService) CopyLocalProduct(ctx context.Context, command productport.CopyLocalProductCommand) (productport.LocalProduct, error) {
	...
	err = service.uow.Within(ctx, func(tx context.Context) error {
		...
		if eventErr := appendLocalProductLifecycleEvent(tx, service.events, "copy", result, sourceProjected.ID, normalized.Actor, reservation, now, eventport.EvProductCreated); eventErr != nil {
			return eventErr
		}
		return service.completeLocalProductSnapshot(tx, receipt.ID, result, now)
	})
...
func (service *LocalProductLifecycleService) DeleteLocalProduct(ctx context.Context, command productport.DeleteLocalProductCommand) (productport.DeleteLocalProductResult, error) {
	...
	err = service.uow.Within(ctx, func(tx context.Context) error {
		...
		// no local entitlement/order/other Product fact references it.
		...
		if eventErr := appendLocalProductLifecycleEvent(tx, service.events, "delete", projected, 0, normalized.Actor, reservation, now, eventport.EvProductUpdated); eventErr != nil {
			return eventErr
		}
...
func appendLocalProductLifecycleEvent(ctx context.Context, events eventport.Appender, action string, product productport.LocalProduct, sourceID productport.ID, actor int64, reservation Reservation, now time.Time, eventType string) error {
