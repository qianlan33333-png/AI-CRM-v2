...
console.log('admin/groupops.html（本地计划与目录边界）');
{
  ...
  click(dom, [...d.querySelectorAll('button')].find((b) => b.textContent.trim() === '查看所有群'));
  await sleep(30);
...
console.log('admin/groupopsDetail.html（真实 HTTP 群目录选择）');
{
  ...
  const open = () => click(dom, [...d.querySelectorAll('button')].find((b) => b.textContent.trim() === '选择群'));
  ...
  const draftName = d.querySelector('#groupOpsName');
  draftName.value = '未保存计划名';
  click(dom, d.querySelector('[data-groupops-panel-button="2"]'));
  ok('V1 左侧步骤只切换一个面板且不跳锚点、不丢未保存草稿', d.querySelector('[data-groupops-panel-button="2"]').classList.contains('is-active') && !d.querySelector('[data-groupops-panel="2"]').hidden && d.querySelector('[data-groupops-panel="1"]').hidden && dom.window.location.hash === '' && draftName.value === '未保存计划名');
  open(); await sleep(30);
...
}
{
  ...
  const d = dom.window.document, test = dom.window.__groupDirectoryTest;
  const action = (name) => click(dom, d.querySelector('[data-gd="' + name + '"]'));
  click(dom, [...d.querySelectorAll('button')].find((b) => b.textContent.trim() === '查看所有群')); await sleep(30);
...
console.log('admin/channelForm.html（完整 OpenAPI 渠道 DTO）');
{
  ...
  click(dom, d.querySelector('[data-channel-panel="2"]'));
  ok('渠道 V1 左侧步骤只切换一个面板且不依赖内联样式', d.querySelector('[data-channel-panel="2"]').classList.contains('active') && !d.querySelector('[data-channel-panel-content="2"]').hidden && d.querySelector('[data-channel-panel-content="1"]').hidden && d.querySelector('#channelName').value === '新客渠道');
