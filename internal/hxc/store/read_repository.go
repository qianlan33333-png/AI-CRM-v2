...
func (r *SenderConfigRepository) ListSenderConfigs(ctx context.Context) ([]hxc.SenderConfig, error) {
	rows, err := r.pool.Query(ctx, `SELECT id,sender_userid,display_name,priority,is_active,created_at,updated_at FROM hxc_sender_configs ORDER BY priority,btrim(sender_userid),id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
