...
func (store *customerAcquisitionLinkReceipts) CompleteCustomerAcquisitionLink(_ context.Context, completion CustomerAcquisitionLinkCompletion) (CustomerAcquisitionLinkReceipt, error) {
	...
	store.receipt.ProviderReceiptDigest = completion.ProviderReceiptDigest
...
func TestCustomerAcquisitionLinkCreateIsDurableAndIdempotent(t *testing.T) {
	...
	provider := &customerAcquisitionLinkProvider{write: wecomport.CustomerAcquisitionLinkWriteResult{Outcome: wecomport.CustomerAcquisitionLinkExecuted, Link: &link, ReceiptDigest: [32]byte{1}, BusinessEndpointDispatched: true, RealExternalCallExecuted: true}}
...
func TestCustomerAcquisitionLinkAttemptedReplayBecomesUnknownWithoutProviderCall(t *testing.T) {
	...
	if err != nil || result.State != CustomerAcquisitionLinkOutcomeUnknown || !result.BusinessEndpointDispatched || provider.updateCalls != 0 {
...
func TestCustomerAcquisitionLinkReadsAndRemainingWritesUseProvider(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			write := wecomport.CustomerAcquisitionLinkWriteResult{Outcome: wecomport.CustomerAcquisitionLinkExecuted, ReceiptDigest: [32]byte{1}, BusinessEndpointDispatched: true, RealExternalCallExecuted: true}
