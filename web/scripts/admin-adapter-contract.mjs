...
await build({ entryPoints: [path.join(root, 'src/api/admin.test.ts'), path.join(root, 'src/api/external_effects.test.ts'), path.join(root, 'src/api/push_observability.test.ts')], bundle: true, platform: 'node', format: 'esm', outdir, logLevel: 'warning' });
try {
