...
[[ "$("$plan" --validate)" = 'final-v1-domain-migration-plan: PASS (schema=132->140 domains=40)' ]] ||
  fail 'baseline validation did not pass'
rendered="$("$plan" --render)"
grep -Fq '"from": 132' <<<"$rendered" || fail 'render omitted schema start'
grep -Fq '"to": 140' <<<"$rendered" || fail 'render omitted schema end'
...
cp "$root/migrations"/001{32,33,34,35,36,37,38,39,40}_*.sql "$fixture/migrations/"
...
printf '%s\n' '-- +goose Up' >"$fixture/migrations/00141_hxc_activation_funnel_local.sql"
if "$fixture/scripts/final_v1_domain_migration_plan.sh" --validate >"$fixture/reject.log" 2>&1; then
  fail 'schema 141 was accepted'
fi
grep -Fq 'schema 141 is forbidden' "$fixture/reject.log" || fail 'schema 141 rejection was not stable'
