...
var hxcChatJobArchivePreflight = flag.Bool("hxc-chat-job-archive-preflight", false, "read only the frozen V2 archive for HXC chat-job observations")

func TestReconciledHXCChatJobArchivePreflight(t *testing.T) {
	if !*hxcChatJobArchivePreflight {
	...
	for _, table := range []struct {
		...
		{ChatJobsTableID, 153, func(row v1archive.ArchivedRow, key []byte) error { _, err := AdaptChatJob(row, key); return err }},
	} {
		count := 0
