...
GOWORK=off GOTOOLCHAIN=local GOFLAGS=-mod=readonly "$go_command" test -race -count=1 -run '^TestCH02TerminalEvidencePG16$' ./internal/externaleffects/store -args -ch02-store-database-url "$ch02_database_url"
