...
func TestLegacyInternalEventDiagnosticsContract(t *testing.T) {
	...
	if !payload.OK || payload.Filters.Status != string(eventport.DeliveryPending) || payload.EventCount != 1 || payload.UndispatchedEventCount != 1 || payload.DeliveryCounts.Pending != 1 || payload.DeliveryCounts.Completed != 0 || len(payload.ConsumerRegistry) != 5 || len(payload.ObservedDomains) != 2 || len(payload.UnobservedDomains) != 3 || payload.RealExternalCallExecuted {
		t.Fatalf("payload=%+v", payload)
	}
}
...
func TestLegacyInternalEventQueryGrammar(t *testing.T) {
	...
	for _, consumer := range []string{eventport.ConsumerAutomationTagTrigger, eventport.ConsumerStatsTagApplied, eventport.ConsumerOperationCycleFact, eventport.ConsumerCloudCampaignFact, eventport.ConsumerOutboundCampaignHandoffFact} {
