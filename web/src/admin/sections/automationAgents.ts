...
async function loadList(root: HTMLElement): Promise<void> {
  ...
  const rows = data.items.map((item) => `<tr>
    ...
    <td style="padding:10px 12px;border-bottom:1px solid #F2F3F5"><span style="display:inline-flex;align-items:center;height:22px;padding:0 8px;border-radius:4px;background:#FFF7E8;color:#8A4B08;font-size:12px">已暂停（执行关闭）</span></td>
  ...
  </tr>`).join('') || '<tr><td colspan="5" style="padding:32px;text-align:center;color:#8F959E">暂无当前自动化配置</td></tr>';
