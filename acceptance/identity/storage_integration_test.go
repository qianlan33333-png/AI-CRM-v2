...
import (
	...
	contactacceptance "github.com/qianlan33333-png/AI-CRM-v2/internal/contact/acceptance"
)
...
func TestIdentityStorageConstraintsAndContactOwnedParents(t *testing.T) {
	...
	parents, err := contactacceptance.CreateIdentityStorageFKParents(ctx, tx, 2)
	if err != nil {
		t.Fatalf("contact-owned FK parents: %v", err)
	}
	if len(parents) != 2 || parents[0] == parents[1] {
		t.Fatalf("contact-owned parent IDs=%v", parents)
	}
...
INSERT INTO identities (
  ...
  $1::bigint, 'unionid', 'wechat-open-platform:acceptance', 'normalized-only', 1,
  'verified', 'acceptance', decode($2::text, 'hex'), 1, now()
) RETURNING id`, parents[0], "00112233445566778899aabbccddeeff").Scan(&identityID)
...
INSERT INTO pending_events (
  kind, identity_ids, candidate_customer_ids, source, policy_version, event_type,
  idempotency_key, occurred_at
) VALUES (
  'attribution', ARRAY[$1::bigint], ARRAY[$2::bigint], 'acceptance', 'identity_v1',
  'event', 'event-key', now()
)`, identityID, parents[1])
	if err != nil {
		t.Fatalf("insert attribution pending event: %v", err)
	}
	if _, err = tx.Exec(ctx, `SAVEPOINT invalid_merge_detail`); err != nil {
		t.Fatalf("save invalid merge detail test: %v", err)
	}
	_, err = tx.Exec(ctx, `
INSERT INTO customer_merges (
  ...
  $1::bigint, $2::bigint, 'auto', 'merge_v1',
  decode($3::text, 'hex'), 1, 'acceptance', $4::jsonb
)`, parents[0], parents[1], "11112222333344445555666677778888", `{"raw_value":"forbidden"}`)
	if err == nil {
	...
	if _, err = tx.Exec(ctx, `ROLLBACK TO SAVEPOINT invalid_merge_detail`); err != nil {
		t.Fatalf("restore invalid merge detail test: %v", err)
	}
}

func TestIdentityReceiptCannotCommitInProgressAndCompletedReceiptIsImmutable(t *testing.T) {
	pool := openIdentityPool(t)
	resetIdentityStorage(t, pool)
	ctx := context.Background()
...
INSERT INTO identity_operation_receipts (
  ...
  'bind', $1::text, decode($2::text, 'hex'), 1,
  decode($3::text, 'hex'), 1, 1
)`, fmt.Sprintf("acceptance-%d", time.Now().UnixNano()), repeatHex("11"), repeatHex("22"))
		if err != nil {
			t.Fatal(err)
		}
		err = tx.Commit(ctx)
	...
	t.Run("completed receipt is immutable", func(t *testing.T) {
		scope := fmt.Sprintf("acceptance-%d", time.Now().UnixNano())
		var receiptID int64
...
INSERT INTO identity_operation_receipts (
  ...
  'bind', $1::text, decode($2::text, 'hex'), 1,
  decode($3::text, 'hex'), 1, 'completed', 1, 'bound', now()
) RETURNING id`, scope, repeatHex("33"), repeatHex("44")).Scan(&receiptID)
...
TRUNCATE identity_operation_receipts, pending_events, customer_merges, identities RESTART IDENTITY`)
	if err != nil {
		t.Fatalf("reset identity storage: %v", err)
	}
}
