...
[[ "$upgrade_waterline" = "38" && "$daily_table" = "1" && "$receipt_table" = "1" && "$daily_index" = "1" ]]
...
[[ "$final_waterline" = "38" && "$receipt_count" = "1" && "$projection_value" = "1" ]]
[[ "$delivery_status" = "completed" && "$automation_receipt" = "1" && "$job_count" = "1" ]]

printf 'P4-W0-L01 migration compatibility: PASS (25/38/25/38, history preserved through current waterline)\n'
