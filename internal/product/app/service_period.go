...
func NewServicePeriodService(uow platformport.UnitOfWork, store ServicePeriodStore, events eventport.Appender) *ServicePeriodService {
...
func (service *ServicePeriodService) CreateServicePeriodProduct(ctx context.Context, command productport.CreateServicePeriodProductCommand) (productport.ServicePeriodProduct, error) {
	...
	err = service.uow.Within(ctx, func(tx context.Context) error {
		...
		if appendErr := service.appendServicePeriodEvent(tx, eventport.EvProductCreated, "create", result, 0, normalized.Actor, actorScope, normalized.IdempotencyKey, now); appendErr != nil {
			return appendErr
		}
...
func (service *ServicePeriodService) CopyServicePeriodProduct(ctx context.Context, command productport.CopyServicePeriodProductCommand) (productport.ServicePeriodProduct, error) {
	...
	err = service.uow.Within(ctx, func(tx context.Context) error {
		...
		if appendErr := service.appendServicePeriodEvent(tx, eventport.EvProductCreated, "copy", result, source.ID, normalized.Actor, actorScope, normalized.IdempotencyKey, now); appendErr != nil {
			return appendErr
		}
		return service.completeServicePeriod(tx, receipt.ID, result, now)
	})
