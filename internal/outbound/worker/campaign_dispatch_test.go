...
func TestCampaignWeComAdapterUsesFrozenAudienceTargetAndCapturesProviderResult(t *testing.T) {
	envelope, err := eer.NewEnvelope(eer.EnvelopeInput{Owner: eer.OwnerOutbound, Kind: eer.KindOutboundMessage, SourceRefDigest: workerDigest("source"), TargetRefDigest: workerDigest("target"), PayloadDigest: workerDigest("payload"), PolicyVersionHash: workerDigest("policy")})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCampaignWeComAdapterFailsClosedForAudienceWithoutFrozenTarget(t *testing.T) {
	envelope, err := eer.NewEnvelope(eer.EnvelopeInput{Owner: eer.OwnerOutbound, Kind: eer.KindOutboundMessage, SourceRefDigest: workerDigest("source"), TargetRefDigest: workerDigest("target"), PayloadDigest: workerDigest("payload"), PolicyVersionHash: workerDigest("policy")})
	if err != nil {
		t.Fatal(err)
	}
	loader := &campaignWeComLoaderSpy{request: outboundport.CampaignDispatchProviderRequest{DispatchID: 41, CustomerID: 7, StepIndex: 1, Content: "hello", PayloadDigest: string(envelope.PayloadDigest()), AudiencePackageID: 33}}
	provider := &campaignWeComProviderSpy{}
	adapter, err := NewCampaignWeComAdapter(loader, provider)
	if err != nil {
		t.Fatal(err)
