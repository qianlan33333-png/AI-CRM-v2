...
func TestOutboundStorageCatalogWaterlineAndIdentity(t *testing.T) {
	...
	var waterline int
	if err := pool.QueryRow(ctx, `SELECT max(version_id) FROM goose_db_version WHERE is_applied`).Scan(&waterline); err != nil || waterline != 27 {
		t.Fatalf("migration waterline=%d err=%v, want 27", waterline, err)
	}
