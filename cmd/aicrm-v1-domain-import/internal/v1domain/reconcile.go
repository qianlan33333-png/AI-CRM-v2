...
func reconcileHXCChatJobEntries(ctx context.Context, tx pgx.Tx, entries []hxcChatJobEntry, run string) (ReconciliationResult, error) {
	if ctx == nil || tx == nil || run == "" {
		return ReconciliationResult{}, ErrInvalidScope
	}
	var count int64
	if err := tx.QueryRow(ctx, "SELECT count(*) FROM public.v1_domain_import_receipts WHERE import_version=$1 AND archive_run_id=$2", HXCChatJobHistoryVersion, run).Scan(&count); err != nil {
...
VALUES($1,$2,$3,$3,$3,0,0,$3,$4) ON CONFLICT(import_version,archive_run_id) DO NOTHING`, HXCChatJobHistoryVersion, run, count, digest)
	if err != nil {
		return ReconciliationResult{}, err
	}
...
FROM public.v1_domain_import_reconciliation_receipts WHERE import_version=$1 AND archive_run_id=$2`, HXCChatJobHistoryVersion, run).Scan(&selected, &receipts, &imported, &archived, &quarantined, &verified, &old)
...
func reconcileCycleObservationEntries(ctx context.Context, tx pgx.Tx, entries []cycleObservationEntry, run string) (ReconciliationResult, error) {
	if ctx == nil || tx == nil || run == "" {
		return ReconciliationResult{}, ErrInvalidScope
	}
	var count int64
	if err := tx.QueryRow(ctx, "SELECT count(*) FROM public.v1_domain_import_receipts WHERE import_version=$1 AND archive_run_id=$2", CycleObservationHistoryVersion, run).Scan(&count); err != nil {
		return ReconciliationResult{}, err
	}
	if count != int64(len(entries)) {
		return ReconciliationResult{}, ErrConflict
	...
	command, err := tx.Exec(ctx, `INSERT INTO public.v1_domain_import_reconciliation_receipts
(import_version,archive_run_id,selected_source_count,receipt_count,imported_count,archived_count,quarantined_count,verified_count,comparison_digest)
VALUES($1,$2,$3,$3,$3,0,0,$3,$4) ON CONFLICT(import_version,archive_run_id) DO NOTHING`, CycleObservationHistoryVersion, run, count, digest)
	if err != nil {
		return ReconciliationResult{}, err
	}
	result.Replayed = command.RowsAffected() == 0
	if result.Replayed {
		...
		var old []byte
		err = tx.QueryRow(ctx, `SELECT selected_source_count,receipt_count,imported_count,archived_count,quarantined_count,verified_count,comparison_digest
FROM public.v1_domain_import_reconciliation_receipts WHERE import_version=$1 AND archive_run_id=$2`, CycleObservationHistoryVersion, run).Scan(&selected, &receipts, &imported, &archived, &quarantined, &verified, &old)
		if err != nil || selected != count || receipts != count || imported != count || verified != count || archived != 0 || quarantined != 0 || !equalBytes(old, digest) {
			return ReconciliationResult{}, ErrConflict
		}
	}
...
func reconcileHXCRuntimeEntries(ctx context.Context, tx pgx.Tx, entries []hxcRuntimeEntry, run string) (ReconciliationResult, error) {
	...
	var count int64
	if err := tx.QueryRow(ctx, "SELECT count(*) FROM public.v1_domain_import_receipts WHERE import_version=$1 AND archive_run_id=$2", HXCRuntimeHistoryVersion, run).Scan(&count); err != nil {
		return ReconciliationResult{}, err
	}
	if count != int64(len(entries)) {
		return ReconciliationResult{}, ErrConflict
	...
	command, err := tx.Exec(ctx, `INSERT INTO public.v1_domain_import_reconciliation_receipts
(import_version,archive_run_id,selected_source_count,receipt_count,imported_count,archived_count,quarantined_count,verified_count,comparison_digest)
VALUES($1,$2,$3,$3,$3,0,0,$3,$4) ON CONFLICT(import_version,archive_run_id) DO NOTHING`, HXCRuntimeHistoryVersion, run, count, digest)
	if err != nil {
		return ReconciliationResult{}, err
	}
	result.Replayed = command.RowsAffected() == 0
	if result.Replayed {
		...
		var old []byte
		err = tx.QueryRow(ctx, `SELECT selected_source_count,receipt_count,imported_count,archived_count,quarantined_count,verified_count,comparison_digest
FROM public.v1_domain_import_reconciliation_receipts WHERE import_version=$1 AND archive_run_id=$2`, HXCRuntimeHistoryVersion, run).Scan(&selected, &receipts, &imported, &archived, &quarantined, &verified, &old)
		if err != nil || selected != count || receipts != count || imported != count || verified != count || archived != 0 || quarantined != 0 || !equalBytes(old, digest) {
			return ReconciliationResult{}, ErrConflict
		}
	}
...
// ReconcileExternalIdentityGap seals authenticated archive rows
