...
[[ "$upgrade_waterline" = "29" && "$daily_table" = "1" && "$receipt_table" = "1" && "$daily_index" = "1" ]]
...
[[ "$final_waterline" = "29" && "$receipt_count" = "1" && "$projection_value" = "1" ]]
[[ "$delivery_status" = "completed" && "$automation_receipt" = "1" && "$job_count" = "1" ]]

printf 'P4-W0-L01 migration compatibility: PASS (25/29/25/29, history preserved through current waterline)\n'
