...
temporary_database='aicrm_test_wecom_contact_profile_effect_00099'
...
[[ "$(psql "$database_url" -X -q -At -c "SELECT max(version_id) FROM goose_db_version WHERE is_applied")" = '99' ]]
...
set +e
"${goose[@]}" down-to 98 >"$guard_output" 2>&1
guard_status=$?
set -e
[[ "$guard_status" -ne 0 ]]
...
printf 'P4 B1-WC02 WeCom contact profile effect PG16.14 acceptance: PASS (99 migration, typed EER unknown/reconcile, durable provider facts, immutable populated down guard; zero network)\n'
