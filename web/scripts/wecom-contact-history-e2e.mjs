...
const bindings = Array.from({ length: 21 }, (_, index) => ({ id: 70 + index, source_person_id: index ? -index : 0, person_history_id: index ? 300 + index : null, identity_id: index ? null : 400, identity_assurance: index ? 'declared' : 'verified', created_at: at, updated_at: at }));
...
globalThis.fetch = async (input, init = {}) => {
  ...
  const source = listEvents || eventDetail ? events : listRelations || relationDetail ? relations : listBindings || bindingDetail ? bindings : directory;
...
try {
  ...
  assert(bindingPage.items[0].source_person_id === 0 && bindingPage.items[1].source_person_id === -1 && bindingPage.items[1].identity_assurance === 'declared' && bindingPage.items[1].identity_id === null, 'binding source or nullable identity fields changed');
