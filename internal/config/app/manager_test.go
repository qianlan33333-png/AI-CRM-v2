...
func (repository *fakeRepository) InsertAudit(context.Context, []byte, configport.SetCommand, []byte, time.Time) (configstore.Audit, bool, error) {
	repository.auditCalls++
	return repository.audit, repository.auditInserted, nil
}
func (repository *fakeRepository) GetAuditByRequestID(context.Context, string) (configstore.Audit, error) {
...
func (appender *fakeAppender) Append(_ context.Context, event eventport.Event) (eventport.EventID, error) {
	appender.calls++
...
func TestSetPersistsAuditAndEventWithoutValueInPayload(t *testing.T) {
	...
	uow := &fakeUoW{}
	repo := &fakeRepository{auditInserted: true, audit: configstore.Audit{ID: 9}}
	events := &fakeAppender{}
	manager := NewManager(uow, repo, events)
...
func TestSetIdempotentReplayAndConflict(t *testing.T) {
	...
	repo := &fakeRepository{
		audit: configstore.Audit{ID: 1, Key: command.Key, NewValue: []byte(`3`), UpdatedBy: command.Actor, RequestID: command.RequestID, UpdatedAt: now},
	}
...
func TestSetPropagatesEventFailure(t *testing.T) {
	...
	repo := &fakeRepository{auditInserted: true, audit: configstore.Audit{ID: 1}}
