...
export function runCapabilityTests(): void {
  ...
  assert(CAPABILITIES.some((cap) => cap.screen.includes('products') && cap.action === '普通商品分享读取、二维码与链接预览' && cap.state === 'real'), 'Ordinary product public detail share must be classified as real');
