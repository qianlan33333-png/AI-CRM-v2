...
func TestServicePeriodCASCopyEnableDisableArchiveAndReferenceRetention(t *testing.T) {
	...
	for _, event := range events.events {
		if event.Type != eventport.EvProductCreated && event.Type != eventport.EvProductUpdated {
			t.Fatalf("unexpected local event type=%q", event.Type)
		}
		if event.CustomerID != 0 {
			t.Fatalf("service-period event exposed customer identity: %+v", event)
		}
	}
}
...
func (events *servicePeriodTestEvents) Append(_ context.Context, event eventport.Event) (eventport.EventID, error) {
	if events.err != nil {
		return 0, events.err
	}
