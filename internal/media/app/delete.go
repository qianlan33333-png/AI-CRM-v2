...
func (service *ImageDeleteService) DeleteImage(ctx context.Context, command ImageDeleteCommand) (ImageDeleteResult, error) {
	...
	err = service.uow.Within(ctx, func(tx context.Context) error {
		...
		if _, appendErr := service.events.Append(tx, eventport.Event{Type: "media.image_deleted", Payload: eventPayload, OccurredAt: now, IdempotencyKey: "media.image_deleted:" + hex.EncodeToString(eventDigest[:])}); appendErr != nil {
			return ErrImageDeleteUnavailable
		}
