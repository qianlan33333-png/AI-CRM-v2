...
type operationCycleTestEvents struct{ events []eventport.Event }

func (events *operationCycleTestEvents) Append(_ context.Context, event eventport.Event) (eventport.EventID, error) {
	events.events = append(events.events, event)
	return eventport.EventID(len(events.events)), nil
}

type operationCycleTestDeliveries struct{ accepted []eventport.EventID }

func (deliveries *operationCycleTestDeliveries) Accept(_ context.Context, eventID eventport.EventID, consumer string) error {
	if consumer != eventport.ConsumerOperationCycleFact {
...
func TestReportIdempotentReplayDoesNotAppendAnotherEvent(t *testing.T) {
	...
	if events.events[0].Type != eventport.EvOperationCycleFact {
