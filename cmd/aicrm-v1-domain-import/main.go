...
func run(args []string, environment appconfig.V1ArchiveRuntime) error {
	...
	if *mode == "reconcile" && *domain != "all" && *domain != "static" && *domain != "finance" && *domain != "channel" && *domain != "service-period" && *domain != "coupon" && *domain != "groupops" && *domain != "message-history" && *domain != "audience-history" {
		return fmt.Errorf("reconcile requires domain=all, static, finance, channel, service-period, coupon, groupops, message-history or audience-history")
	}
	...
	if *mode == "import" && (*domain == "static" || *domain == "finance" || *domain == "channel" || *domain == "coupon" || *domain == "service-period" || *domain == "groupops" || *domain == "message-history" || *domain == "audience-history") {
...
func validDomain(value string) bool {
	return value == "campaign" || value == "survey" || value == "media" || value == "radar" || value == "shop" || value == "all" || value == "static" || value == "finance" || value == "channel" || value == "service-period" || value == "coupon" || value == "groupops" || value == "message-history" || value == "audience-history"
}
