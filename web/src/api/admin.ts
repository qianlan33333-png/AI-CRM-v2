...
export function questionnairePageDto(questionnaire: LegacyQuestionnaire): Questionnaire { return { resourceId: questionnaire.id, publicPath: questionnaire.public_path, name: questionnaire.title, assess: questionnaire.assessment_enabled, off: questionnaire.is_disabled, action: questionnaire.status, created: questionnaire.created_at, count: String(questionnaire.submission_count) }; }
...
export const couponPageDto = (value: unknown): Coupon => { const source = obj(value); const x = obj(source.coupon || value); return { resourceId: Number(x.id), name: text(x.name), code: 'c-' + text(x.id), discountAmountTotal: Number(x.discount_amount_total || 0), totalIssueLimit: Number(x.total_issue_limit || 0), perUserIssueLimit: Number(x.per_user_issue_limit || 1), claimStartsAt: text(x.claim_starts_at, ''), claimEndsAt: text(x.claim_ends_at, ''), validityMode: x.validity_mode === 'fixed_range' ? 'fixed_range' : 'relative_days', useStartsAt: typeof x.use_starts_at === 'string' ? x.use_starts_at : null, useEndsAt: typeof x.use_ends_at === 'string' ? x.use_ends_at : null, relativeValidityDays: x.relative_validity_days == null ? null : Number(x.relative_validity_days), instructions: text(x.instructions, ''), targetRefs: list(x, 'target_refs').map(String), version: Number(x.version || 0), off: `¥${(Number(x.discount_amount_total || 0) / 100).toFixed(2)}`, scope: list(x, 'target_refs').join('、') || '—', window: `${text(x.claim_starts_at)} 至 ${text(x.claim_ends_at)}`, issue: `${text(x.issued_count, '0')} / ${text(x.total_issue_limit, '0')}`, status: text(x.status), tone: toneFor(x.status) }; };
