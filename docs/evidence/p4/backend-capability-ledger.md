...
| `00084_ai_audience_local_configuration_closure.sql` — AI Audience Local Configuration Closure (**V2-native local package; not a legacy-flow completion claim**) | Appends immutable template/filter snapshots, versioned automation binding, hashed local sender references, and PII-minimal send-record projections. `LEGACY-S06-022`/`024`/`026` remain `NOT_STARTED/NOT_RUN`: this package intentionally omits template catalogue/preview, directory sync, legacy send-record detail, and every provider or send path. | human session plus global admin/ops `segments.read` or `segments.write`; all mutations require CSRF + Idempotency-Key and use local CAS/receipt/UoW semantics. The local snapshot and projection routes never expose content, recipient/sender IDs, credentials, or provider responses. | `internal/segment/legacyaudience/local_configuration_*_test.go`; an isolated PG16 exact-84 migration run with `CI_TEST_DATABASE_URL` verifies append-only/PII negatives and the populated down guard. | `none / NOT_EXECUTED`; no preview, agent start, queue/enqueue, Provider call, send, acceptance, or delivery assertion. Legacy data import remains unimplemented and is not inferred from this V2-native schema. |
