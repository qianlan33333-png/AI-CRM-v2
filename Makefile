...
.PHONY: p4-j01-coupon-acceptance p4-coupon-ab-acceptance p4-i03-order-acceptance p4-order-ab-acceptance p4-message-archive-ab-acceptance p4-operation-cycle-ab-acceptance p4-automation-agents-ab-acceptance p4-adminops-jobs-ab-acceptance p4-s04-legacy-health-acceptance
...
p4-operation-cycle-ab-acceptance:
	...
	@/usr/bin/env -u BASH_ENV -u ENV GOWORK=off GOTOOLCHAIN=local GOFLAGS=-mod=readonly $(GO) test -race -count=1 -timeout=180s ./acceptance/operationcycle -args -database-url "$${P4OPERATIONCYCLE_TEST_DATABASE_URL}" -expected-waterline 43
...
p4-s04-legacy-health-acceptance:
	@/usr/bin/env -u BASH_ENV -u ENV GOWORK=off GOTOOLCHAIN=local GOFLAGS=-mod=readonly $(GO) test -race -count=1 -timeout=45s ./internal/platform/legacyhealth ./internal/config ./cmd/aicrm
