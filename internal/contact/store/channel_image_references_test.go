...
func TestChannelImageReferenceIDsRejectsNonCanonicalArrayElements(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			got, err := channelImageReferenceIDs([]byte(test.raw))
			if test.err {
				if err == nil {
					t.Fatalf("channelImageReferenceIDs(%s) succeeded", test.raw)
				}
				return
			}
			if err != nil || !reflect.DeepEqual(got, test.want) {
				t.Fatalf("channelImageReferenceIDs(%s) = %#v, %v; want %#v, nil", test.raw, got, err, test.want)
			}
		})
	}
}
