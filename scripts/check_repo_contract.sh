...
acceptance_base_ref="HEAD"
if [[ -n "${GITHUB_BASE_REF:-}" ]] && git rev-parse --verify --quiet "origin/${GITHUB_BASE_REF}" >/dev/null; then
  acceptance_base_ref="$(git merge-base HEAD "origin/${GITHUB_BASE_REF}")"
elif git diff --cached --quiet -- docs/ci/go-acceptance-manifest.tsv && git rev-parse --verify --quiet HEAD^ >/dev/null; then
  acceptance_base_ref="HEAD^"
fi
CI_ACCEPTANCE_VALIDATE_ONLY=1 CI_ACCEPTANCE_BASE_REF="$acceptance_base_ref" \
...
for anchor in \
  'candidate-title' 'prohibited-merge' 'candidate-evidence' \
  'missing-mapping' 'missing-acceptance' 'missing-http' 'missing-store' 'missing-migration' \
  'formal-no-schema' 'no-schema-without-slice' 'no-schema-forged-business-text' 'no-schema-without-slice-declaration' \
  'policy-only' 'policy-self-description' 'policy-test-self-description' 'policy-receipt-self-description' \
  'openapi-checker-self-description' 'implementation-not-wired'; do
...
! git show :docs/feature-matrix.csv | grep -Eq 'agent-runs|automation_trigger_receipts' ||
  fail "P4-W0-D01 falsely added the API-only receipt to the UI feature matrix"
