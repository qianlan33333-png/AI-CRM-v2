...
for argument in "$@"; do
  case "$argument" in
    ...
    *) fail 'only --from=135 --to=142 --runtime-env-file=<absolute-file> are accepted' ;;
  esac
done

[[ "$from" = 135 && "$to" = 142 ]] || fail 'only one bounded Goose migration from 135 to 142 is accepted'
...
env -u BASH_ENV -u ENV GOOSE_DRIVER=postgres GOOSE_DBSTRING="$database_url" \
  "$goose_command" -dir "$migrations" up-to 142
