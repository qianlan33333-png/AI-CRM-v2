...
func TestFinalRouterMountsGroupOpsPageCarriersWithAdminRead(t *testing.T) {
	router, auth := legacySurveyRouter(t, &legacySurveyStub{item: legacySurveyItem()})
	tests := []struct {
		path     string
		location string
	}{
		{path: automationhttp.GroupOpsPlansPath, location: "/?legacy_admin_path=%2Fadmin%2Fautomation-conversion%2Fgroup-ops%2Fui"},
	...
	for _, test := range tests {
		...
		router.ServeHTTP(response, legacyRequest(http.MethodGet, test.path, legacyToken(192)))
...
func TestFinalRouterRejectsGroupOpsPageMethodsBeforeAuthentication(t *testing.T) {
	router, auth := legacySurveyRouter(t, &legacySurveyStub{item: legacySurveyItem()})
	for _, path := range []string{automationhttp.GroupOpsPlansPath, "/admin/automation-conversion/group-ops/plans/42"} {
