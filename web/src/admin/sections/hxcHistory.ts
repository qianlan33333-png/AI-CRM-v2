...
const kinds: HxcHistoryKind[] = ['meta', 'snapshot', 'activation', 'lead', 'batch', 'sender_config', 'send_record', 'chat_job'];
...
function url(kind: HxcHistoryKind, id?: number): string { return `funnel.html?hxc_history=1&history_kind=${kind}${id === undefined ? '' : `&history_id=${id}`}`; }
function parse(options: Options): { kind: HxcHistoryKind; id?: number } {
  const query = new URLSearchParams(location.search); const allowed = new Set(['hxc_history', 'history_kind', 'history_id']);
  for (const key of query.keys()) if (!allowed.has(key) || query.getAll(key).length !== 1) throw new Error('HXC 历史参数无效');
  const kind = options.kind ?? query.get('history_kind') ?? 'snapshot'; const id = options.historyID ?? query.get('history_id') ?? undefined;
  ...
  return { kind: kind as HxcHistoryKind, id: id === undefined ? undefined : Number(id) };
}
  ...
  const load = async (offset = 0): Promise<void> => {
    ...
    try {
      if (input.id !== undefined) {
        ...
        results.innerHTML = `<p><a href="${url(input.kind)}">返回历史列表</a></p><article data-hxc-history-id="${item.id}" style="border:1px solid #DEE0E3;border-radius:8px;padding:14px">${summary(input.kind, item)}</article>`;
        return;
      }
      const page = await readHxcHistory(input.kind, offset, 20);
