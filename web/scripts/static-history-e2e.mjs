...
const kinds = ['GroupInvite', 'ProductPageSlice', 'CycleStrategy', 'CycleVersion', 'CycleDocument'];
const fields = {
...
const endpoint = { GroupInvite: 'group-invites', ProductPageSlice: 'page-slices', CycleStrategy: 'cycle-strategies', CycleVersion: 'cycle-versions', CycleDocument: 'cycle-documents' };
...
globalThis.fetch = async (input, init = {}) => {
  ...
  const item = { id: 7, source_id: -9, source_key_digest: digest, source_payload_digest: digest, ...fields[kind] };
  const offset = Number(url.searchParams.get('offset') ?? '0');
  const body = fail ? '{"raw":"NO"}' : JSON.stringify({ source: 'v1_history', read_only: true, real_external_call_executed: false, ...(isDetail ? { item } : { items: offset === 0 ? Array.from({ length: 20 }, (_, index) => ({ ...item, id: index + 1 })) : [{ ...item, id: 21 }], total: 21, limit: 20, offset }) });
...
try {
  ...
  if (calls.length < 6 || !calls.every(({ init }) => init.method === 'GET' && init.credentials === 'include' && init.body === undefined)) throw new Error('static history made a non-GET request');
