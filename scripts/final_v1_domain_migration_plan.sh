...
[[ "$latest" = '145' ]] || fail "final schema must be 145, found ${latest:-none}"

grep -Fq '"from": 132' "$manifest" || fail 'manifest must start from schema 132'
grep -Fq '"to": 145' "$manifest" || fail 'manifest must end at schema 145'
...
if [[ "$mode" = '--render' ]]; then
  ...
  printf 'final-v1-domain-migration-plan: PASS (schema=132->145 domains=%s)\n' "$domain_count"
fi
